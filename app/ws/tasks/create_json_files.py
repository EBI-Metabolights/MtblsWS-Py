...
def update_extension_set(assay, file_indices, file_extensions):
    ...
    if (
        "assayTable" in assay
        and assay["assayTable"]
        and "data" in assay["assayTable"]
        and assay["assayTable"]["data"]
    ):
...
class StudyJsonExporter(Resource):
    ...
    def get(self):
        ...
        json_path = os.path.join(
            study_settings.mounted_paths.reports_root_path,
        ...
        with DBManager.get_instance().session_maker() as db_session:
            ...
            if not studies:
                raise MetabolightsDBException("There is no study")
        ...
        futures = [
            executor.submit(self.read_study_folder, study_folders, json_path, m_study)
            for m_study in m_study_list
        ]
...
class PublicStudyJsonExporter(Resource):
    ...
    def get(self):
        ...
        with DBManager.get_instance().session_maker() as db_session:
            query = db_session.query(Study)
            if not study_id:
                query = query.filter(Study.status == StudyStatus.PUBLIC.value).order_by(
                    Study.acc
                )
            else:
                query = query.filter(
            ...
            if not studies:
                raise MetabolightsDBException("There is no public study")
