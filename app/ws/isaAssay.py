    ...
    def delete(self, study_id, assay_file_name):
        ...
        for assay in isa_study.assays:  # ToDo, check if we can delete the correct assay if the file is missing
            ...
            if assay_file_name == a_file:
                ...
                try:
                    remove_file(study_location, a_file, study_id=study_id, study_path=study_location, always_remove=True)  # We have to remove active metadata files
                    if maf_name is not None:
                        remove_file(study_location, maf_name, study_id=study_id,
                                    study_path=study_location, always_remove=True)
                except:
