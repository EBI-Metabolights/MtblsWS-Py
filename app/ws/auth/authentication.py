...
def validate_token_in_request_body(content):
    if (
        not content or isinstance(content, str)
...
class AuthUser(Resource):
    ...
    def post(self):
        ...
        if (
            ...
            and "user" in response.headers
            and response.headers["jwt"]
        ):
            username = response.headers["user"]
            try:
                UserService.get_instance().validate_username_with_submitter_or_super_user_role(
                    username
                )
                ...
                return make_response(
                    jsonify({"content": "invalid", "message": None, "err": None}), 403
                )
        return response
