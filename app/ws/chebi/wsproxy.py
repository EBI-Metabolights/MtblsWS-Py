...
from app.ws.chebi.types import (
    RelationshipType,
    SearchCategory,
    StarsCategory,
...
class ChebiWsProxy(object):
    ...
    def setup(self):
        ...
        client_settings = Settings(
            strict=self.settings.chebi_ws_strict,
        ...
        try:
            ...
            return client.bind(
                self.settings.chebi_ws_wsdl_service,
    ...
    @ws_fault_exception_handler
    def get_lite_entity_list(
        self,
        search_text: str,
        search_category: SearchCategory,
        maximum_results: int,
        stars: StarsCategory,
    ) -> Union[None, List[LiteEntity]]:
        result = self.service.getLiteEntity(
            search_text, search_category.value, maximum_results, stars.value
        )
        serialized_result = helpers.serialize_object(result, dict)
        if serialized_result:
            result = [
                LiteEntity.model_validate(data) for data in serialized_result if data
            ]
            return result
        return None

    @ws_fault_exception_handler
    def get_complete_entity_by_list(
        self, chebi_id_list: List[str]
    ) -> Union[None, List[Entity]]:
    ...
    def get_ontology_parents(
        self, chebi_id: str
    ) -> Union[None, List[OntologyDataItem]]:
        service_result = self.service.getOntologyParents(chebi_id)
        serialized_result = helpers.serialize_object(service_result, dict)
        if serialized_result:
            result = [
                OntologyDataItem.model_validate(data)
                for data in serialized_result
                if data
            ]
            return result
        return None
    ...
    def get_all_ontology_children_in_path(
        self, chebi_id: str, relationship_type: RelationshipType, structure_only: bool
    ) -> Union[None, List[LiteEntity]]:
        service_result = self.service.getAllOntologyChildrenInPath(
            chebi_id, relationship_type.value, structure_only
        )
        serialized_result = helpers.serialize_object(service_result, dict)
        if serialized_result:
            result = [
    ...
    def get_structure_search(
        ...
        service_result = self.service.getStructureSearch(
            structure,
            structure_type,
