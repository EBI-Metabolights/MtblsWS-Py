...
from app.ws.chebi.search import commons, utils
...
class ChebiSearchManager(object):
        ...
        if "chebi" in database_id.lower():
            try:
                ...
                if not complete_entity:
                    return abort(
                        404, message=f"Entity not found with ChEBI id {database_id}"
                    )
    ...
    def get_compound_by_inchi(self, inchi: str):
        ...
        return self._search_by_category(
            ...
            CuratedMetaboliteTable.INCHI_INDEX,
        )
    ...
    def get_compound_by_smiles(self, smiles: str):
        ...
        return self._search_by_category(
            compound_smiles,
            smiles,
            SearchCategory.SMILES,
            CuratedMetaboliteTable.SMILES_INDEX,
        )

    def _search_by_category(
        self,
        filtered_search_value,
        search_value,
        category: SearchCategory,
        curation_table_index: int,
    ):
        response = CompoundSearchResponseModel()
        commons.search_hits_with_search_category(
            filtered_search_value,
            category,
            curation_table_index,
            response,
            StarsCategory.ALL,
            self.ws_proxy,
    ...
    def get_compound_by_name(self, compound_name: str):
        ...
        if not compound_name or compound_name.lower() in ("unknown", "unidentified"):
            compound_search_result = CompoundSearchResultModel(
                search_resource=SearchResource.CURATED, name="unknown"
            )
    ...
    def _search_hits_from_list_and_chebi_only(
        self, compound_name: str, response: CompoundSearchResponseModel
    ):
        curated_metabolite_table = self.curated_metabolite_table

        name_match = curated_metabolite_table.get_matching_rows(
            CuratedMetaboliteTable.COMPOUND_INDEX, compound_name
        )
        if name_match:
            ...
            try:
                ...
                if not complete_entity:
                    return abort(
                        404, message=f"Entity not found with ChEBI id {chebi_id}"
                    )
                compound_search_result_v2 = CompoundSearchResultModel(
                    name=complete_entity["name"],
                    inchi=complete_entity["default_structure"]["standard_inchi"] or "",
    ...
    def _search_and_fill_by_name(self, compound_name, name_match, response):
        ...
        if name_match_chebi_id:
            index = utils.find_term_index_in_source(
                name_match_compound_name, compound_name
            )
            ...
            if result:
                if result.formula:
                    name_match_formula = name_match[
                        CuratedMetaboliteTable.FORMULA_INDEX
                    ]
        ...
        if compound_name.endswith("ate"):
            ...
            if ontology_items:
                for item in ontology_items:
                    ...
                    if (
                        data_item_type == "is conjugate acid of"
                        or data_item_type == "is conjugate base of"
                    ):
    ...
    def _search_name_and_synonyms(self, compound_name, result):
        compound_name_to_compare = utils.remove_few_characters_for_consistency(
            compound_name
        )
        for lite_entity in result:
            entity_name = lite_entity.chebiAsciiName
            entity_name_to_compare = utils.remove_few_characters_for_consistency(
                entity_name
            )
        ...
        for lite_entity in result:
            ...
            if complete_entity:
                for item in complete_entity.synonyms:
                    item_name = item.data
                    item_name_to_compare = utils.remove_few_characters_for_consistency(
                        item_name
                    )
