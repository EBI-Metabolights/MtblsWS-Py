...
from app.ws.isa_tools_v1.model import (
    Assay,
    Characteristic,
    Comment,
    DataFile,
...
def dump(isa_obj, output_path, i_file_name='i_investigation.txt',
    ...
    def _build_roles_str(roles):
        ...
        for role in roles:
            ...
            roles_source_refs += \
                (role.term_source if role.term_source else '') + ';'
        if len(roles) > 0:
    ...
    def _build_protocols_section_df(protocols=list()):
        ...
        for i, protocol in enumerate(protocols):
            ...
            for parameter in protocol.parameters:
                ...
                parameters_source_refs \
                += (parameter.parameter_name.term_source
            ...
            for component in protocol.components:
                ...
                component_types_source_refs += \
                    component.component_type.term_source \
            ...
            if protocol.protocol_type.term_source:
                protocol_type_term_source_name = protocol.protocol_type.term_source
...
def write_study_table_files(inv_obj, output_dir):
    ...
    for study_obj in inv_obj.studies:
        ...
        path_attrs = _longest_path_and_attrs(paths)
        if path_attrs:
            for node in path_attrs:
                if isinstance(node, Source):
                    olabel = "Source Name"
    ...
    if isinstance(x.value, (int, float)) and x.unit:
        if isinstance(x.unit, OntologyAnnotation):
            ...
            df_dict[label + ".Unit.Term Source REF"][-1] = \
                x.unit.term_source if x.unit.term_source else ""
        ...
        df_dict[label + ".Term Source REF"][-1] = \
            x.value.term_source if x.value.term_source else ""
