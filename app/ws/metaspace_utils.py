...
def aws_download_files(
    mtspc_obj, output_dir, extension, data_type="binary", use_path=False
):
    for sample in mtspc_obj:
        aws_bucket, aws_path, file_name = get_filename_parts(sample, extension)
        logger.info(
            "Getting file %s %s %s (%s)", aws_bucket, aws_path, file_name, data_type
        )
...
def aws_get_annotations(
    mtspc_obj, output_dir, database=None, fdr=None, sm_instance=None
):
    ...
    for sample in mtspc_obj:
        ...
        for an in ds.annotations(fdr=fdr, database=database):
            ...
            img = ds.isotope_images(sf=an[0], adduct=an[1])[
                0
            ]  # get image for this molecule's principle peak
    ...
    if dataset_ids and sm_instance:
        ...
        for ds_id in dataset_ids:
            try:
                ...
                logger.error(
                    "Could not find dataset " + ds_id + " in the METASPACE database"
                )
                continue
            settings = get_settings()
            fdr = settings.metaspace.configuration.metaspace_fdr
            db_name = settings.metaspace.configuration.metaspace_database
            annotation = db.get_annotations(
                fdr=fdr, db_name=db_name, datasetFilter={"ids": ds_id}
            )
            ...
            if dataset:
                ...
                if dataset.metadata:
                    ...
                    )  # json.dumps(dataset.metadata.json)
...
def annotate_metaspace(
    study_id=None, sm=None, metaspace_projects=None, metaspace_datasets=None
):
...
def import_metaspace(
    ...
    std_description = (
        "Please update abstract of this METASPACE generated study " + study_id
    )
    use_path = False
    ...
    if mtspc_obj:
        aws_download_files(
            mtspc_obj, output_dir, "imzML", data_type="utf-8", use_path=use_path
        )
        aws_download_files(
            mtspc_obj, output_dir, "ibd", data_type="binary", use_path=use_path
        )
        aws_get_annotations(mtspc_obj, output_dir, sm_instance=sm_instance)
        aws_get_images(
            mtspc_obj, output_dir, use_path=use_path, sm_instance=sm_instance
        )

    get_all_files(
        dataset_ids,
        [".imzML", ".ibd", ".jpg", ".jpeg", ".png"],
        output_dir,
        use_path=use_path,
