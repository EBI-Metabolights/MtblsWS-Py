...
def get_files_for_validation(study_id, study_metadata_files_path) -> FileSearchResult:
    ...
    referenced_files = get_referenced_file_set(
        study_id=study_id, metadata_path=study_metadata_files_path
    )
    metadata_and_data_files = get_all_study_metadata_and_data_files(
        study_metadata_files_path, exclude_list=exclude_list
    )
    search_result = evaluate_files_in_detail(metadata_and_data_files, referenced_files)
    return search_result


def get_all_files_from_filesystem(
    study_id,
    obfuscation_code,
    study_location,
    directory=None,
    ...
    logger.info(
        "Getting list of all files for MTBLS Study %s. Study folder: %s.",
        study_id,
        study_location,
    )
    ...
    logger.info(
        "Listing study files for "
        + study_id
        + " took %s seconds" % round(time.time() - s_start_time, 2)
    )
...
def get_all_files(
    ...
    try:
        files, latest_update_time = get_file_information(
            study_location=path,
            path=path,
            directory=directory,
            include_raw_data=include_raw_data,
...
def get_file_information(
    ...
    try:
        ...
        tree_file_list = []
        try:
            tree_file_list, static_file_found = list_directories(
                study_location,
                dir_list=[],
                base_study_location=study_location,
                short_format=short_format,
                validation_only=validation_only,
                include_sub_dir=include_sub_dir,
                static_validation_file=static_validation_file,
        ...
        if validation_only and short_format and not static_file_found:
            for file_name in assay_file_list:
                if (
                    os.path.isfile(os.path.join(study_location, file_name))
                    and file_name not in tree_file_list
                ):
        ...
        for entry in flatten_list(tree_file_list):
            ...
            folder = None
            if (
                short_format and not static_file_found
            ):  # The static file contains more info and is fast to read
                file_name = entry
            else:
            ...
            if time.time() > end_time:
                logger.error(
                    "Listing files in folder %s, timed out after %s seconds",
                    path,
                    timeout_secs,
                )
            ...
            if not file_name.startswith("."):  # ignore hidden files on Linux/UNIX:
                if not include_raw_data:  # Only return metadata files
                    if file_name.startswith(("i_", "a_", "s_", "m_")):
                        file_time, raw_time, file_type, status, folder = get_file_times(
                            path, file_name, validation_only=validation_only
                        )
                else:
                    file_time, raw_time, file_type, status, folder = get_file_times(
                        path,
                ...
                if directory:
                    if file_name.startswith(("i_", "a_", "s_", "m_")):
                        status = (
                            "old"  # metadata files in a sub-directory are not active
                        )
                    ...
                    if latest_update_time == "":
                        ...
                        latest_update_time = (
                            ...
                            if datetime.datetime.strptime(
                                latest_update_time, "%B %d %Y %H:%M:%S"
                            )
                            > datetime.datetime.strptime(file_time, "%B %d %Y %H:%M:%S")
                            else file_time
                        )
    except Exception as e:
        logger.error(
            "Error in listing files under " + path + ". Last file was " + file_name
        )
        logger.error(str(e))
...
def get_basic_files(
    study_location, include_sub_dir, assay_file_list=None, metadata_only=False
):
    file_list = []
    start_time = time.time()

    if include_sub_dir:
        file_list = list_directories_full(
            study_location, file_list, base_study_location=study_location
        )
        ...
        for entry in os.scandir(study_location):
            if not entry.name.startswith("."):
                ...
                if (
                    metadata_only
                    and fname.startswith(("i_", "a_", "s_", "m_"))
                    and (ext == ".txt" or ext == ".tsv")
                ):
                    file_type, status, folder = map_file_type(
                        file_name, study_location, assay_file_list=assay_file_list
                    )
                else:
                    file_type, status, folder = map_file_type(
                        file_name, study_location, assay_file_list=assay_file_list
    ...
    logger.info(
        "Basic tree listing for all files for "
        + study_location
        + " took %s seconds" % round(time.time() - start_time, 2)
    )
    return file_list
...
def list_directories_full(
    file_location, dir_list, base_study_location, assay_file_list=None
):
    for entry in os.scandir(file_location):
        name = entry.path.replace(base_study_location + os.sep, "")
        file_type, status, folder = map_file_type(
            entry.name, file_location, assay_file_list=assay_file_list
        )
        dir_list.append(
            {
                "file": name,
    ...
    if static_validation_file and os.path.isfile(static_validation_file):
        ...
        for entry in os.scandir(file_location):
            ...
            if not entry.name.startswith("."):
                ...
                if not include_raw_data and not name.startswith(
                    ("i_", "a_", "s_", "m_")
                ):
                    continue
                ...
                if entry.is_dir():
                    if not include_sub_dir:
                        ...
                        if (
                            file_type in ["raw", "derived"] and validation_only
                        ):  # or validation only?
                            continue
                        else:
                            dir_list.extend(
...
def write_audit_files(study_location_or_study_id):
    ...
    try:
        ...
        search_pattern = os.path.join(
            maintenance_task.study_audit_files_path, "20??-??-??_??-??-??_BACKUP"
        )
        ...
        if folders:
            try:
                ...
                datetime.datetime.strptime(
                    name.replace("_BACKUP", ""), date_time_separted_format
                )
        ...
        if last_audit_folder and metadata_files_signature:
            ...
            audit_folder_signature = maintenance_task.read_hash_file(
                metadata_files_signature_root_path=hashes_path
            )
            if not audit_folder_signature:
                audit_folder_signature = maintenance_task.read_hash_file(
                    metadata_files_signature_root_path=last_audit_folder
                )
