...
class StudyRevisionService:
    ...
    @staticmethod
    def check_dataset_integrity(study_id: str, metadata_files_path: str, data_files_path: str):
        ...
        metadata_signature = Path(f"{metadata_files_path}/HASHES/metadata_files_signature.txt")
        if metadata_signature.exists():
            expected_metadata_signature = Path(f"{metadata_files_path}/HASHES/metadata_files_signature.txt").read_text().strip()
            metadata_files_signature, metadata_files_hashes = maintenance_task.calculate_metadata_files_hash(search_path=metadata_files_path)
            ...
            logger.warning(f"metadata file signature file does not exist: {metadata_signature}")
            
        data_files_signature = Path(f"{metadata_files_path}/HASHES/data_files_signature.txt")
        if data_files_signature.exists():
            expected_data_file_signature = Path(f"{metadata_files_path}/HASHES/data_files_signature.txt").read_text().strip()
            data_files_signature, data_files_hashes, _ = StudyRevisionService.create_data_file_hashes(study, search_path=data_files_path)
            ...
            logger.warning(f"data files signature file does not exist: {data_files_signature}")
        
    @staticmethod
    def create_data_file_hashes(study: Study, data_files_hashes_file: str=None, data_files_signature_file: str=None, search_path=None, copy_paths: Union[None,  List[str]] = None):
        """
        ...
        try:
            ...
            if data_files_hashes_file or data_files_signature_file:
                if not os.path.exists(revision_folder_path):
                    raise Exception(f"Revision folder does not exists: {revision_folder_name}")
            parent = os.path.join(metadata_revisions_path, revision_folder_name, "HASHES")
            ...
            if not data_files_signature_file:
                data_files_signature_file = os.path.join(parent, "data_files_signature.txt")
            logger.info("Calculate data file hashes.")
            folder_hash, data_file_hashes, search_path = maintenance_task.calculate_data_file_hashes(search_path=search_path)
            ...
            if copy_paths:
                for copy_path in copy_paths:
                    ...
                    data_files_signature_file = os.path.join(copy_path, "data_files_signature.txt")
                    with open(data_files_signature_file, "w") as f:
                        f.write(f"{folder_hash}")
    ...
    def create_audit_folder(study: Study, folder_name: str, metadata_files_path: str= None, audit_folder_root_path: str=None):
        study_id = study.acc
        study_status = StudyStatus(study.status)
        public_release_date = study.releasedate
        submission_date = study.submissiondate
        ...
        return maintenance_task.create_audit_folder(metadata_files_path=metadata_files_path, 
                                                    audit_folder_root_path=audit_folder_root_path, 
                                                    folder_name=folder_name, stage=None)
    ...
    def update_investigation_file_for_revision(study_id: str):
        ...
        revision_comments = [c for c in isa_study.comments if c.name == "Dataset Revision"]
        revision_datetimes = [c for c in isa_study.comments if c.name == "Dataset Revision Date"]
        revision_comments.extend(revision_datetimes)
        if study.revision_number > 0:
            comment = model.Comment(name="Dataset Revision", value=str(study.revision_number))
            isa_study.comments.append(comment)
            revision_datetime = ""
            if study.revision_datetime:
                revision_datetime = study.revision_datetime.strftime("%Y-%m-%d")
            comment = model.Comment(name="Dataset Revision Date", value=revision_datetime)
            isa_study.comments.append(comment)
            ...
            with DBManager.get_instance().session_maker() as db_session:
                try:
                    ...
                    study_revision: StudyRevision = query.filter(StudyRevision.accession_number == study_id, 
                                                                StudyRevision.revision_number == revision_number ).first()
                    ...
                    db_session.commit()
                    logger.info(f'New revision task started: ({study_id} Revision {revision_number}) ')
                except Exception as e:
                    db_session.rollback()
                    raise e

    @staticmethod
    def update_study_revision_task_status(study_id: str, 
                                          revision_number: int,
    ...
    def get_study_revisions(study_id: str):
        ...
        with DBManager.get_instance().session_maker() as db_session:
            try:
                query = db_session.query(StudyRevision)
                result = query.filter(StudyRevision.accession_number == study_id).order_by(StudyRevision.revision_number.desc()) .all()
                revisions = []
                for item in result:
                    study_revision: StudyRevision = item
                    model = StudyRevisionModel.model_validate(study_revision, from_attributes=True)
    ...
    def get_study_revision(study_id: str, revision_number: int):
        ...
        with DBManager.get_instance().session_maker() as db_session:
            try:
                query = db_session.query(StudyRevision)
                result = query.filter(StudyRevision.accession_number == study_id, StudyRevision.revision_number == revision_number).order_by(StudyRevision.revision_number.desc()) .first()
                if result:
                    model = StudyRevisionModel.model_validate(result, from_attributes=True)
                    return model
                return None
            except Exception as e:
                raise e
    ...
    @staticmethod
    def increment_study_revision(study_id: str, 
        ...
        with DBManager.get_instance().session_maker() as db_session:
            try:
                ...
                if study.status not in {StudyStatus.PRIVATE.value, StudyStatus.INREVIEW.value, StudyStatus.PUBLIC.value}:
                    raise MetabolightsException(http_code=401, message="Study is not public")
                ...
                es_service.reindex_study_with_task(study_id=study_id, 
                                                   user_token=None, 
                                                   include_validation_results=False,
                                                   sync=False)
                logger.info(f'New revision (Revision {study.revision_number}) is created for study {study_id}')
            except Exception as e:
                db_session.rollback()
                raise e
    ...
    def create_revision_folder(study: Study):
        ...
        try:
            ...
            dest_path = maintenance_task.create_audit_folder(audit_folder_root_path=metadata_revisions_path, 
    ...
    def _delete_revision_folder(study: Study):
        ...
        try:
            ...
            revision_path = os.path.join(metadata_revisions_root_path, revision_folder_name)
            ...
            if revision_number > 1:
                previous_revision_path = os.path.join(metadata_revisions_root_path, f"{study_id}_{(revision_number-1):02d}") 
        ...
        with DBManager.get_instance().session_maker() as db_session:
            try:
                ...
                if int(study.revision_number) != revision_number:
                    raise MetabolightsException(http_code=401, message=f"Only the latest revision can be deleted. Current revision number: {study.revision_number}")
                
                query = db_session.query(StudyRevision)
                study_revision: StudyRevision = query.filter(StudyRevision.accession_number == study_id, 
                                                             StudyRevision.revision_number == revision_number ).first()
                if study_revision.status == StudyRevisionStatus.IN_PROGRESS.value:
                    raise MetabolightsException(http_code=401, message="Revision task is in progress, cannot be deleted.")
                ...
                if revision_number == 1:
                    ...
                    study.releasedate = (study.first_private_date.replace(tzinfo=None) + datetime.timedelta(days=365))
                    if study.status != StudyStatus.PRIVATE.value:
                        study.status = StudyStatus.PRIVATE.value
                    ElasticsearchService.get_instance()._delete_study_index(study_id=study_id, 
                                                            ignore_errors=True)
                else:
                    previous_revision: StudyRevision = query.filter(StudyRevision.accession_number == study_id, 
                                                                StudyRevision.revision_number == (revision_number - 1) ).first()
                ...
                logger.info(f'Study {study_id} revision ({study.revision_number + 1}) is deleted.')
            except Exception as e:
                db_session.rollback()
                raise e
            return StudyRevisionModel.model_validate(study_revision, from_attributes=True)
