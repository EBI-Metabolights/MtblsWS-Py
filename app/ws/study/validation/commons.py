...
def validate_samples(isa_study, isa_samples, validation_schema, file_name, override_list, comment_list, sample_name_list,
    ...
    if too_short:
        add_msg(validations, val_section, "Organism name is missing or too short (<4 characters)", error, file_name,
                val_sequence=9, log_category=log_category)
    if not isa_study.factors:
        add_msg(validations, val_section, "Define at least one Factor Value in sample sheet.", error, file_name,
                val_sequence=20.1, log_category=log_category)
    else:
        factor_names_in_study = {str(x.name) for x in isa_study.factors}
        extra_factor_values_in_sample = []
        factor_values_in_sample_sheet = set()
        for header_name in sample_header:
            if header_name.startswith("Factor Value["):
                match = re.match(factor_value_pattern, header_name)
                if match:
                    factor = match.groups()[1]
                    factor_values_in_sample_sheet.add(factor)
                    if factor not in factor_names_in_study:
                        extra_factor_values_in_sample.append(factor)
                    values = set(isa_samples[header_name].unique())
                    values.discard(None)
                    values.discard("")
                    if len(values) == 0:
                        add_msg(validations, val_section, f"Empty column Factor Value[{factor}].", error, file_name,
                            val_sequence=20.2, log_category=log_category)
        if extra_factor_values_in_sample:
            factors = ", ".join(extra_factor_values_in_sample)
            add_msg(validations, val_section, f"Factor values defined in sample sheet should be added to study description. Please add {factors} factors in Descriptors tab.", error, file_name,
                    val_sequence=20.3, log_category=log_category)
        missing_columns = [x for x in factor_names_in_study if x not in factor_values_in_sample_sheet]
        if missing_columns:
            columns = ", ".join([f"Factor Value[{x}]" for x in missing_columns])
            add_msg(validations, val_section, f"Factor in study description should be defined in sample sheet as a column. Please add {columns} columns and complete values.", error, file_name,
                val_sequence=20.4, log_category=log_category)
