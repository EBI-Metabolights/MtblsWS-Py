            ...
            with db_session:
                ...
                if category_name:
                    filter_clause = lambda query: query.filter(
                        MlStat.page_section == category_name
                    )
                    db_parameters: List[MlStat] = (
                        filter_clause(query)
                        .order_by(MlStat.sort_order.asc(), MlStat.str_name.asc())
                        .all()
                    )
                else:
                    db_parameters: List[MlStat] = query.order_by(
                        MlStat.page_section.asc(),
                        MlStat.sort_order.asc(),
                        MlStat.str_name.asc(),
                    ).all()
                m_params = [
                    MetabolightsStatisticsModel.model_validate(db_param).model_dump()
