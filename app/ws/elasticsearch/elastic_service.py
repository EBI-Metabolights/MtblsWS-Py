...
from app.ws.elasticsearch.schemes import (
    Booster,
    Facet,
    FacetLine,
    SearchQuery,
...
class ElasticsearchService(object):
    ...
    def initialize_client(self):
        ...
        relative_path = settings.configuration.elasticsearch_all_mappings_json.lstrip(
            "./"
        ).lstrip("/")
        ...
        relative_path = (
            settings.configuration.elasticsearch_compound_mappings_json.lstrip(
                "./"
            ).lstrip("/")
        )
        mappings_file_path = os.path.join(app_path, relative_path)
        ...
        self._client.indices.put_mapping(
            ...
            doc_type=self.DOC_TYPE_COMPOUND,
            body=body,
            ignore=[404, 400],
        )
        relative_path = settings.configuration.elasticsearch_study_mappings_json.lstrip(
            "./"
        ).lstrip("/")
        mappings_file_path = os.path.join(app_path, relative_path)
        with open(mappings_file_path) as f:
    ...
    def get_instance(cls):
        if not cls.instance:
            ...
            cls.instance = ElasticsearchService(
                settings=settings,
                db_manager=db_mananager,
    ...
    def search(self, query: SearchQuery):
                ...
                for item in result["hits"]["hits"]:
                    ...
                    if item["_type"] and "_source" in item and item["_source"]:
                        if item["_type"].lower() == "study":
                            result_item = models.StudyModel.model_validate(
                                item["_source"]
                            )
                        elif item["_type"].lower() == "compound":
                            result_item = models.ESMetaboLightsCompound.model_validate(
                                item["_source"]
                            )
                    if result_item:
            ...
            if result and "aggregations" in result and result["aggregations"]:
                for facet_name in result["aggregations"]:
                    if facet_name in facets:
                        ...
                        if (
                            "buckets" in result["aggregations"][facet_name]
                            and result["aggregations"][facet_name]["buckets"]
                        ):
                            ...
                            for bucket in buckets:
                                if "key" in bucket and "doc_count" in bucket:
                                    if bucket["key"] not in current_facet["lines"]:
                                        ...
                                        current_facet["lines"][bucket["key"]] = (
                                            new_facet_line
                                        )
                                        current_facet["facet"].lines.append(
                                            new_facet_line
                                        )
                                    current_facet["lines"][
                                        bucket["key"]
        ...
        return {
            "content": search_result.model_dump(),
    ...
    def build_search_body(self, query: SearchQuery) -> str:
        ...
        if not search_text:
            ...
            if filtered:
                body["query"] = {
                    "filtered": {
                        "query": {
                            "bool": {"must": {"query_string": {"query": search_text}}}
                        }
                    }
                }
            else:
            ...
            boosters = [
                {
                    "term": {
                        boost.fieldName: {"value": search_text, "boost": boost.boost}
                    }
                }
        ...
        if not query.searchUser.isAdmin:
            ...
            filter = {
                "or": {
                    "filters": [
                        {"term": {"studyStatus": "PUBLIC"}},
                        {"term": {"users.userName": query.searchUser.id}},
                    ]
                }
            }
        ...
        try:
            with DBManager.get_instance().session_maker() as db_session:
                metabolite = (
                    ...
                    .filter(RefMetabolite.acc == compound_id)
                    .first()
                )
                ...
                compound = models.MetaboLightsCompoundIndexModel.model_validate(
                    metabolite
                )
                organisms = set()
