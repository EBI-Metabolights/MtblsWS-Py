...
class StudyAssayTypeReports(Resource):
    ...
    def post(self):
        ...
        studytype = request.args.get("studytype", "").strip()
        ...
        msg = AnalyticalMethodBuilder(
            original_study_location=study_root_location,
            studytype=studytype,
            slim=slim,
            ...
            for studyID, study_info in j_file["data"].items():
                ...
                if not studyStatus:
                    ...
                    if (
                        d >= start_date
                        and d <= end_date
                        and status.lower() in studyStatus
                    ):
                        data_res.update({studyID: study_info})
                    else:
                        continue
            ...
            if study_id:
                ...
                for tech, value in res1["techniques"].items():
                    if tech in j_data["data"]["techniques"]:
                        print(tech)
                        j_data["data"]["techniques"][tech] += (
                            value  # res['techniques'][tech]
                        )
                    else:
                ...
                j_data["data"]["study_type"]["targeted"] += res2["study_type"][
                    "targeted"
                ]
                j_data["data"]["study_type"]["untargeted"] += res2["study_type"][
                    "untargeted"
                ]
                j_data["data"]["study_type"]["targeted_untargeted"] += res2[
                    "study_type"
                ]["targeted_untargeted"]
...
class EuropePMCReport(Resource):
    ...
    def get(self):
        ...
        inputs = {"google_sheet_id": google_sheet_id}
        if google_sheet_id:
