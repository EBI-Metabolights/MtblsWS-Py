...
def get_ftp_folder_access_status(study_id):
    ...
    try:
        if ftp_private_storage.remote.does_folder_exist(ftp_private_study_folder):
            permission = ftp_private_storage.remote.get_folder_permission(
            ...
            if permission == Acl.AUTHORIZED_READ_WRITE:
                ...
                if permission == Acl.AUTHORIZED_READ or permission == Acl.READ_ONLY:
                    ...
                    return {
                        "Access": access,
                        "status": "error",
                        "message": "Permission status is unknown",
                    }
        else:
            return {
                "Access": access,
                "status": "error",
                "message": "There is no folder",
            }
        return {"Access": access, "status": "success"}
    except OSError as e:
        logger.error(
            "Error in getting the permission for %s : %s",
            ftp_private_study_folder,
            str(e),
        )
...
def toogle_ftp_folder_permission(study_id):
    study = StudyService.get_instance().get_study_by_acc(study_id)
    ...
    try:
        ...
        if ftp_private_storage.remote.does_folder_exist(ftp_study_folder):
            ...
            if permission == Acl.AUTHORIZED_READ_WRITE:
                ftp_private_storage.remote.update_folder_permission(
                    ftp_study_folder, Acl.AUTHORIZED_READ
                )
                access = "Read"
            elif permission == Acl.AUTHORIZED_READ or permission == Acl.READ_ONLY:
                ftp_private_storage.remote.update_folder_permission(
                    ftp_study_folder, Acl.AUTHORIZED_READ_WRITE
                )
                access = "Write"
        ...
        logger.error(
            f"Error in updating the permission for {ftp_study_folder} Error {str(e)}"
        )
