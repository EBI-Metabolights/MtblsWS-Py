...
class BuilderPerformanceTracker:
    ...
    def push(self, key, val):
        ...
        if all(isinstance(el, type(val)) for el in getattr(self, key)):
            ...
            raise TypeError(
                f"attempt to push val {val} of type {type(val)} to list of type {type(getattr(self, key))}"
            )

    def start_timer(self, timer_name: str):
        self._timers.update({timer_name: {"start_time": time.time(), "end_time": None}})
    ...
    def get_duration(self, timer_name: str) -> float:
        return (
            self._timers[timer_name]["end_time"]
    ...
    def report_all_timers(self):
        return [
            f"\n {name} took {str(self.get_duration(name))} sec"
            for name, timer_dict in self._timers.items()
        ]
