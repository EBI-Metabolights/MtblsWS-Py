...
def run_chebi_pipeline(
    ...
    if run_on_cluster:
        ...
        param = (
            " -d 'classyfire_search="
            + str(classyfire_search).lower()
            + "' -d 'annotation_file_name=#FILE_NAME#' -d 'update_study_maf="
            + str(update_study_maf).lower()
            + "' "
        )
        cmd = (
            cmd
            + param
            + " -i -H \\'Accept: application/json\\' -H \\'Content-Type: application/json\\' -H \\'user_token: "
            + user_token
            + "\\' '"
        )
        cmd = (
            cmd
            + get_settings().chebi.pipeline.chebi_pipeline_url
            + study_id
    ...
    if annotation_file_name is None:
        ...
        for file in study_files:
            ...
            if file_name.startswith("m_") and file_name.endswith(".tsv"):
                ...
                (
                    ...
                    pubchem_file,
                    dime_db_api_active,
                    cactus_search_status,
                    opsin_search_status,
                    chemspider_search_status,
                    unichem_search_status,
                    classyfire_search_status,
                    classy_file_downoaded,
                    chebi_search_status,
                    chebi_master_list_initialised,
                    chebi_search_manager_initiated,
                ) = search_and_update_maf(
                    study_id,
                    study_metadata_location,
                    file_name,
                    classyfire_search,
        ...
        )  # Replace the dummy file name reference in URL
        if run_on_cluster:
            # create param file
            print_log("Starting cluster job for ChEBI pipeline: " + cmd)
            status, message, job_out, job_err, log_file = submit_job(
                True, account=user_email, job_cmd=cmd, job_params=None, log=False
            )
        ...
        return {
            "in_rows": maf_len,
            "out_rows": new_maf_len,
            "pubchem_file": pubchem_file,
            "DimeDB API hit": dime_db_api_active,
            "OPSIN API hit": opsin_search_status,
            "Cactus API hit": cactus_search_status,
            "ChemSpider API hit": chemspider_search_status,
            "Unichem API hit": unichem_search_status,
            "CHEBI API hit": chebi_search_status,
            "Classyfire API hit": classyfire_search_status,
            "Classyfire SDF downloaded": classy_file_downoaded,
            "CHEBI Master List initialised": chebi_master_list_initialised,
            "CHEBI search manager initiated": chebi_search_manager_initiated,
        }
...
def read_sdf_file(sdf_file_name):
    ...
    try:
        ...
        print_log(
            "    -- Could not read MOL structure in file "
            + sdf_file_name
            + ". Error: "
            + str(e)
        )
...
def split_rows(maf_df, annotation_file=None):
    ...
    try:
        ...
        print_log(
            'Warning: Checking for pipelines in the MAF failed. Trying to read using Excel "ISO-8859-1" encoding',
            mode="warning",
        )
    ...
    if pipe_found:
        ...
        try:
            ...
            print_log(
                "    -- ERROR: MAF splitter failed!. Error: " + str(e), mode="error"
            )
            return maf_df
    else:
...
def split_maf_df(maf_file_name):
    ...
    try:
        args = [
            os.path.join(resource_folder, "maf-splitter.jar"),
            maf_file_name,
        ]  # Any number of args
    ...
    try:
        ...
        print_log(
            "Could not read and/or remove 'split' file " + split_file + ". " + str(e)
        )
...
def jar_wrapper(*args):
    process = subprocess.Popen(
        ["java", "-jar"] + list(args), stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
    process.wait()
...
def check_maf_for_pipes(study_location, annotation_file_name):
    ...
    annotation_folder_path = os.path.join(
        study_location,
...
def check_if_unknown(comp_name):
    ...
    try:
        if int(comp_name) > 0:
            print_log(
                "    -- Compound name is only a number? Ignoring '" + comp_name + "'"
            )
        ...
        print_log(
            "    -- ERROR: Testing if compound name is only a number failed '"
            + comp_name
            + "'. Error: "
            + str(e),
            mode="error",
...
def get_pubchem_substance(comp_name, res_type):
    ...
    try:
        ...
        for idx, result in (
            enumerate(results["InformationList"]["Information"]) if results else []
        ):
            print_log(
                "    -- Found PubChem Substance(s) for '"
                + comp_name
                + "' ("
                + res_type
                + "), search record #"
...
def get_existing_values(df, comp_name):
    ...
    row = df[
        (df.metabolite_identification == comp_name) & (df.search_flag == "1")
        | (df.search_flag == "1.0")
    ]
    if len(row.index) >= 1:
        row.loc[
            row[maf_compound_name_column] == comp_name, ["row_id", "search_flag"]
        ] = "", "1"
        return row[
            :1
        ]  # Only return the 1st row, otherwise the replace function in Pandas will "paste" all rows
...
def get_sample_details(study_id, user_token, study_location):
    ...
    print_log(
        "Reading ISA-Tab sample file took %s seconds"
    ...
    for idx, sample in sample_df.iterrows():
        ...
        try:
            ...
            if (
                "blank" in org_part.lower() or org_part == " []"
            ):  # ToDo: Do not need blank or other controls
                org_part = ""
        except Exception as e:
            logging.exception(e)
            org_part = ""
        ...
        try:
            ...
            ]  # There may not always be an ontology for this, so two calls
...
def convert_to_chebi_onto(onto_term):
    ...
    chebi_onto = (
        chebi_onto.replace("ncbitaxon:", "ncbi:")
        .replace("_", ":")
        .replace("txid", "")
        .upper()
    )
...
def populate_sample_rows(pubchem_df, study_id, user_token, study_location):
    ...
    for idx, row in newdf.iterrows():  # Loop and add the different unique sample rows
        if (
            row.iloc[0] == "" and all_organisms and len(all_organisms) > idx
        ):  # Only add if database is not known and sample file is valid.
            s_row = all_organisms[idx]
            org_parts = s_row.split("|")
            newdf.iloc[idx, [org_pos, org_part_pos, strain_pos]] = (
                org_parts[0],
                org_parts[1],
                org_parts[2],
            )
...
def search_and_update_maf(
    study_id: str,
    study_metadata_location: str,
    ...
    curated_metabolite_table: CuratedMetaboliteTable = (
        CuratedMetaboliteTable.get_instance()
    )
    ...
    if (
        obfuscation_code
    ):  # So the curators can FTP new files into the private upload folder for the study
        ftp_private_annotation_folder = os.path.join(
            study_id.lower() + "-" + obfuscation_code,
            settings.internal_files_symbolic_link_name,
            anno_sub_folder,
        )
        ftp_private_storage = StorageService.get_ftp_private_storage()
        create_annotation_folder_on_remote_storage(
            ftp_private_storage, ftp_private_annotation_folder
        )
    ...
    if (
        exiting_pubchem_file
    ):  # The has already been split and this is an existing "pubchem" file
    ...
    for idx, row in short_df.iterrows():
        ...
        final_cid = None
        db_acc = str(
            pubchem_df.iloc[idx, get_idx("DATABASE_ACCESSION", pubchem_df_headers)]
        )
        if exiting_pubchem_file:
            if (
                str(row[3]).rstrip(".0") == "1"
            ):  # This is the already searched flag in the spreadsheet
                search = False
            final_cid = row[4]
            org_row_id = row[5]
            alt_name = row[2]
            ...
            pubchem_df.iloc[idx, get_idx("ID", pubchem_df_headers)] = "temp_" + str(
                org_row_id
            )
        ...
        if not exiting_pubchem_file:
            ...
            pubchem_df.iloc[idx, get_idx("NAME", pubchem_df_headers)] = (
                original_comp_name
            )
            # if not database_id:
        ...
        if final_cid and final_cid.endswith(".mol") and final_inchi:
            ...
            if classyfire_search_id:
                pubchem_df.iloc[
                    row_idx, get_idx("classyfire_search_id", pubchem_df_headers)
                ] = str(classyfire_search_id)
        ...
        if search and comp_name and check_if_unknown(comp_name):
            ...
            if existing_row and len(existing_row.index) > 0:
                print_log(
                    "    -- Updating row(s) for compound name '"
                    + original_comp_name
                    + "' (alt name = '"
                    + alt_name
                    + "')"
                ...
                pubchem_df.update(
                    pubchem_df[[maf_compound_name_column]].merge(existing_row, "left")
                )
                # f not exiting_pubchem_file:
                pubchem_df.iloc[row_idx, get_idx("row_id", pubchem_df_headers)] = (
                    row_idx + 1
                )  # Update Row id again, not use the copied row
                # if not database_id:
                #     pubchem_df.iloc[row_idx, get_idx('combination')] = row_idx + 1  # Cluster sort field

                pubchem_df.iloc[
                    row_idx, get_idx(alt_name_column, pubchem_df_headers)
                ] = alt_name  # Add in the original alt name again
                pubchem_df.iloc[row_idx, get_idx("search_type", pubchem_df_headers)] = (
                    "copy_existing_row"  # Search category/type for logging
                )
                changed = True
            else:
                start_time = time.time()
                ...
                comp_name = (
                    comp_name.strip()
                )  # Remove leading/trailing spaces before we search
                comp_name = (
                    comp_name.replace("Î´", "delta").replace("?", "").replace("*", "")
                )
                if "[" in comp_name:
                    comp_name = comp_name.replace("[U]", "").replace("[S]", "")
                    comp_name = re.sub(re.escape(r"[iso\d]"), "", comp_name)

                search_res = wsc.get_maf_search(
                    "name", comp_name
                )  # This is the standard MetaboLights aka Plugin search
                if not search_res:
                ...
                if search_res and search_res["content"]:
                    ...
                    if (
                        chemical_formula and "-" in chemical_formula
                    ):  # MTBLS search adds the charge at the end
                    ...
                    pubchem_df.iloc[row_idx, get_idx(database_identifier_column)] = (
                        database_identifier
                    )
                    pubchem_df.iloc[row_idx, get_idx("chemical_formula")] = (
                        chemical_formula
                    )
                    pubchem_df.iloc[row_idx, get_idx("smiles")] = smiles
                    pubchem_df.iloc[row_idx, get_idx("inchi")] = inchi
                    pubchem_df.iloc[row_idx, get_idx(maf_compound_name_column)] = (
                        original_comp_name  # name / metabolite_identification from MAF
                    )
                    if not exiting_pubchem_file:
                        pubchem_df.iloc[
                            row_idx, get_idx("row_id", pubchem_df_headers)
                        ] = ""
                        # if not database_id:
                        #     pubchem_df.iloc[row_idx, get_idx('combination')] = ''  # Sorting cluster
                    pubchem_df.iloc[
                        row_idx, get_idx("search_flag", pubchem_df_headers)
                    ] = ""
                    pubchem_df.iloc[
                        row_idx, get_idx(alt_name_column, pubchem_df_headers)
                    ] = alt_name  # alt_name, for us to override the submitted name

                    if name:
                        pubchem_df.iloc[
                            row_idx, get_idx("search_type", pubchem_df_headers)
                        ] = "plugin_search"  # Search category/type for logging
                ...
                if (
                    not chebi_found
                ):  # We could not find this in ChEBI, let's try other sources
                    ...
                    (
                        ...
                        comp_name, search_type="name", search_category="compound"
                    )

                    pubchem_df.iloc[
                        row_idx, get_idx("search_type", pubchem_df_headers)
                    ] = where_found  # Search category/type for logging
                    ...
                    if pc_inchi_key:
                        ik = pc_inchi_key
                        print_log(
                            "    -- Searching ChemSpider using PubChem InChIKey, not Cactus"
                        )
                    csid = get_csid(ik)

                    pubchem_df.iloc[
                        row_idx, get_idx("iupac_name", pubchem_df_headers)
                    ] = pc_name  # PubChem name
                    ...
                    if not final_cid:
                        final_cid = pc_cid
                    pubchem_df.iloc[
                        row_idx, get_idx(final_cid_column_name, pubchem_df_headers)
                    ] = final_cid  # Final PubChem CID (and other external id's manually added)
                    pubchem_df.iloc[
                        row_idx, get_idx("pubchem_cid", pubchem_df_headers)
                    ] = pc_cid  # PubChem CID
                    if not final_cid:
                        final_cid = get_pubchem_cid_on_inchikey(
                            cactus_stdinchikey, opsin_stdinchikey
                        )
                        pubchem_df.iloc[
                            row_idx, get_idx(final_cid_column_name, pubchem_df_headers)
                        ] = final_cid  # Final PubChem CID should now be the cactus or opsin
                    ...
                    if final_cid and (not pc_name or not pc_synonyms):
                        (
                            ...
                            final_cid, search_type="cid", search_category="cid"
                        )
                        if pc_name:
                            pubchem_df.iloc[
                                row_idx, get_idx("iupac_name", pubchem_df_headers)
                        ...
                        if pc_synonyms:
                            pubchem_df.iloc[
                                row_idx, get_idx("pubchem_synonyms", pubchem_df_headers)
                            ] = pc_synonyms

                    pubchem_df.iloc[
                        row_idx, get_idx("pubchem_cid_ik", pubchem_df_headers)
                    ] = pc_cid  # PubChem CID
                    pubchem_df.iloc[row_idx, get_idx("csid_ik", pubchem_df_headers)] = (
                        csid  # ChemSpider ID (CSID) from INCHI
                    )
                    final_smiles, source_found = get_ranked_values(
                        pc_smiles, cactus_smiles, opsin_smiles, None
                    )  # final smiles
                    pubchem_df.iloc[
                        row_idx, get_idx("final_smiles", pubchem_df_headers)
                    ] = final_smiles
                    final_inchi, source_found = get_ranked_values(
                        pc_inchi, cactus_inchi, opsin_inchi, None
                    )  # final inchi
                    pubchem_df.iloc[
                        row_idx, get_idx("final_inchi", pubchem_df_headers)
                    ] = final_inchi
                    final_inchi_key, source_found = get_ranked_values(
                        pc_inchi_key, cactus_stdinchikey, opsin_stdinchikey, None
                    )  # final inchikey
                    pubchem_df.iloc[
                        row_idx, get_idx("final_inchi_key", pubchem_df_headers)
                    ] = final_inchi_key
                    ...
                    if final_inchi_key and csid == "":
                        ...
                        pubchem_df.iloc[
                            row_idx, get_idx("csid_ik", pubchem_df_headers)
                        ] = csid

                    if not pc_inchi:
                        # We don't have a PubChem compound, but we may find a uncurated substance record
                        print_log(
                            "    -- Searching for PubChem substance as we did not find a compound"
                        )
                        (
                            pc_name,
                            pc_inchi,
                            ...
                            comp_name, search_type="name", search_category="substance"
                        )

                        if pc_name:
                            pubchem_df.iloc[
                                row_idx, get_idx("iupac_name", pubchem_df_headers)
                            ] = pc_name  # PubChem substance name
                        ...
                        if not cactus_stdinchikey and pc_inchi_key:
                            print_log(
                                "    -- Searching ChemSpider using PubChem Substance InChIKey"
                            )
                    ...
                    pubchem_df.iloc[
                        row_idx, get_idx("pubchem_smiles", pubchem_df_headers)
                    ] = pc_smiles  # pc_smiles
                    pubchem_df.iloc[
                        row_idx, get_idx("cactus_smiles", pubchem_df_headers)
                    ] = cactus_smiles  # cactus_smiles
                    pubchem_df.iloc[
                        row_idx, get_idx("opsin_smiles", pubchem_df_headers)
                    ] = opsin_smiles  # opsin_smiles
                    pubchem_df.iloc[
                        row_idx, get_idx("pubchem_inchi", pubchem_df_headers)
                    ] = pc_inchi  # PubChem inchi
                    pubchem_df.iloc[
                        row_idx, get_idx("cactus_inchi", pubchem_df_headers)
                    ] = cactus_inchi  # Cactus inchi
                    pubchem_df.iloc[
                        row_idx, get_idx("opsin_inchi", pubchem_df_headers)
                    ] = opsin_inchi  # Opsin inchi
                    pubchem_df.iloc[
                        row_idx, get_idx("pubchem_inchi_key", pubchem_df_headers)
                    ] = pc_inchi_key  # PubChem stdinchikey
                    pubchem_df.iloc[
                        row_idx, get_idx("cactus_inchi_key", pubchem_df_headers)
                    ] = cactus_stdinchikey  # cactus_stdinchikey
                    pubchem_df.iloc[
                        row_idx, get_idx("opsin_inchi_key", pubchem_df_headers)
                    ] = opsin_stdinchikey  # opsin_stdinchikey
                    pubchem_df.iloc[
                        row_idx, get_idx("pubchem_formula", pubchem_df_headers)
                    ] = pc_formula  # PubChem formula
                    pubchem_df.iloc[
                        row_idx, get_idx("pubchem_synonyms", pubchem_df_headers)
                    ] = pc_synonyms  # PubChem synonyms
                    pubchem_df.iloc[
                        row_idx, get_idx("pubchem_first_synonym", pubchem_df_headers)
                    ] = first_synonym  # PubChem first synonym, ie. the compound name
                    # if not cactus_synonyms:
                    # cactus_synonyms = alt_name

                    pubchem_df.iloc[
                        row_idx, get_idx("cactus_synonyms", pubchem_df_headers)
                    ] = cactus_synonyms  # add cactus_synonyms

                    if final_cid:
                        glytoucan_id = get_glytoucan_id(final_cid, glytoucan_file_df)
                        if glytoucan_id:
                            pubchem_df.iloc[
                                row_idx, get_idx("glytoucan_id", pubchem_df_headers)
                            ] = glytoucan_id
                    ...
                    if cactus_synonyms:
                        if pc_synonyms is None:
                            ...
                            cactus_synonyms = get_valid_synonyms(
                                cactus_synonyms, pc_synonyms
                            )
                            if cactus_synonyms:
                                db_acc = db_acc + ";" + cactus_synonyms
                    # Cactus and PubChem synonyms for SDF export
                    pubchem_df.iloc[
                        row_idx, get_idx("direct_parent", pubchem_df_headers)
                    ] = ""  # direct_parent from ClassyFire
                    pubchem_df.iloc[
                        row_idx, get_idx("source_metabolights", pubchem_df_headers)
                    ] = study_id  # MTBLS accession
                    ...
                    elif get_relevant_synonym(
                        comp_name
                    ):  # Do we know the source, ie. KEGG, HMDB?
                        (
                            chebi_id,
                            inchi,
                            inchikey,
                    ...
                    if chebi_id:
                        ...
                        print_log(
                            ...
                            + "', ChEBI id "
                            + database_identifier
                            + " based on direct search, type: "
                            + search_type
                        )
                        pubchem_df.iloc[
                            row_idx, get_idx(database_identifier_column)
                        ] = database_identifier
                        pubchem_df.iloc[row_idx, get_idx("chemical_formula")] = (
                            chemical_formula
                        )
                ...
                print_log(
                    "    -- Search took %s seconds" % round(time.time() - start_time, 2)
                )
                changed = True
        else:
            pubchem_df.iloc[row_idx, get_idx(search_flag, pubchem_df_headers)] = (
                "1"  # Search flag set so we don't search for unknown again
            )
            print_log(
                "    -- Skipping. Already found or no database id/compound name to search for: '"
                + database_id
                + "' '"
                + comp_name
                + "'",
        ...
        if not org_row_id:
            pubchem_df.iloc[row_idx, get_idx("row_id", pubchem_df_headers)] = (
                row_idx + 1
            )  # Row id added if copy and no search results
        if db_acc:
            db_acc = ";".join(unique_list(db_acc.split(";")))
            pubchem_df.iloc[
                row_idx, get_idx("DATABASE_ACCESSION", pubchem_df_headers)
            ] = db_acc.rstrip(";")

        final_inchi_key = pubchem_df.iloc[
            idx, get_idx("final_inchi_key", pubchem_df_headers)
        ]
        if final_inchi_key and len(final_inchi_key) > 0:
            unichem_id = pubchem_df.iloc[
                row_idx, get_idx("unichem_id", pubchem_df_headers)
            ]
            unichem_id_filtered = unichem_id
            if not unichem_id:
                unichem_id = processUniChemResponseAll(final_inchi_key)
                pubchem_df.iloc[row_idx, get_idx("unichem_id", pubchem_df_headers)] = (
                    unichem_id.rstrip(";")
                )
            ...
            if not cas_id:
                ...
                if not cas_id:
                    pubchem_synonyms = pubchem_df.iloc[
                        row_idx, get_idx("pubchem_synonyms", pubchem_df_headers)
                    ]
                    if pubchem_synonyms:
                        cas_id = get_cas_from_synonyms(pubchem_synonyms)
                    if not cas_id:
                        cactus_synonyms = pubchem_df.iloc[
                            row_idx, get_idx("cactus_synonyms", pubchem_df_headers)
                        ]
                        if cactus_synonyms:
                ...
                if dime_db_api_active:
                    ...
                    if not dimedb_search_status:
                        ...
                        if not dimedb_search_status:
                            ...
                            if not dimedb_search_status:
                                print_log(
                                    " Dime DB API is down, skipping for rest of the run. "
                                )
                                dime_db_api_active = False
                            else:
                                dime_id = get_dime_db(final_inchi_key)
                                pubchem_df.iloc[
                                    row_idx, get_idx("dime_id", pubchem_df_headers)
                                ] = dime_id
                        else:
                            dime_id = get_dime_db(final_inchi_key)
                            pubchem_df.iloc[
            ...
            if update_db_acc:
                update_db_acc = (
                    update_db_acc.rstrip(";") + ";" + unichem_id_filtered.rstrip(";")
                )
            else:
        ...
        if changed and row_idx > 0 and row_idx % 20 == 0:  # Save every 20 rows
            ...
            print_log("------------------------------------")
            print_log(
                "Updating PubChem and annotated file. Record "
                + str(idx + 1)
                + " of "
                + str(new_maf_len)
            )
    ...
    if not exiting_pubchem_file:
        ...
        pubchem_df = populate_sample_rows(
            pubchem_df, study_id, user_token, study_metadata_location
        )
    ...
    classy_file_downoaded = update_sdf_file_info(
        pubchem_df,
        annotated_study_location,
        short_file_name + classyfire_end,
        classyfire_search,
        study_id,
        pubchem_file,
        pubchem_df_headers,
    ...
    write_tsv(pubchem_df, pubchem_file)
    concatenate_sdf_files(
        pubchem_df,
        annotated_study_location,
        short_file_name + complete_end,
        run_silently,
    )
    ...
    print_log(
        "ChEBI pipeline Done. Overall it took %s seconds"
        % round(time.time() - first_start_time, 2)
    )

    folder_name = get_settings().chebi.pipeline.chebi_annotation_sub_folder
    internal_files_path = os.path.join(
        study_metadata_location, settings.internal_files_symbolic_link_name
    )
    zip_file_folder_path = os.path.dirname(anno_sub_folder_path)
    compressed_chebi_annotations_folder_path = os.path.join(
        zip_file_folder_path, f"{folder_name}.zip"
    )
    try:
        if os.path.exists(compressed_chebi_annotations_folder_path):
            compressed_chebi_annotations_folder_archive_path = os.path.join(
                zip_file_folder_path, f"{folder_name}_old.zip"
            )
            shutil.move(
                compressed_chebi_annotations_folder_path,
                compressed_chebi_annotations_folder_archive_path,
            )
    except Exception as e:
    ...
    try:
        with zipfile.ZipFile(
            compressed_chebi_annotations_folder_path, "w", zipfile.ZIP_DEFLATED
        ) as zipf:
    ...
    if obfuscation_code:
        ...
        target_ftp_path = os.path.join(
            get_settings().hpc_cluster.datamover.mounted_paths.cluster_private_ftp_root_path,
            ftp_private_folder,
            settings.internal_files_symbolic_link_name,
        )
        include_list = [
...
def update_original_maf(
    maf_df=None,
    pubchem_df=None,
    original_maf_name=None,
    study_location=None,
    update_study_maf=None,
):
...
def create_annotation_folder_on_remote_storage(storage: Storage, folder_loc):
    ...
    try:
        if not storage.remote.does_folder_exist(folder_loc):
            print_log(
                f"Creating ChEBI folder {folder_loc} on storage {storage.get_name()}"
            )
        storage.remote.create_folder(
            folder_loc, acl=Acl.AUTHORIZED_READ_WRITE, exist_ok=True
        )
    except Exception as e:
        print_log(str(e))
...
def sync_annotation_folder_with_remote_storage(storage: Storage, ftp_folder):
    ...
    try:
        storage.sync_from_local(
            source_local_folder=None,
            target_folder=ftp_folder,
            ignore_list=None,
...
def update_sdf_file_info(
    pubchem_df,
    study_location,
    classyfire_file_name,
    classyfire_search,
    study_id,
    pubchem_file_name,
    pubchem_df_headers,
    ...
    for idx, row in pubchem_df.iterrows():
        ...
        if cid and not db_id.startswith("CHEBI:"):
            cluster_ids.append(
                row_id
            )  # Keep count of the number of ORGANISM sections to add to ChEBI SDF file
            if cid.endswith(".mol"):
                fname = cid
            else:
            ...
            if not os.path.isfile(full_file):
                try:
                    if "MTBLS" not in cid:
                        print_log(
                            "       -- PubChem file for CID "
                            + cid
                            + " is missing or final id has been added manually to the spreadsheet. Trying to download again"
                        )
                        pcp.download("SDF", full_file, cid)
                except Exception as e:
                    print_log(str(e))

            mtbls_sdf_file_name = os.path.join(
                study_location, "mtbls_" + cid + pubchem_sdf_extension
            )
            if cluster_itr == 1:  # First time, remove some of the PubChem parameters
                mtbls_sdf_file_name = remove_pubchem_sdf_parameters(
                    study_location, fname
                )

            # Now, get the classyFire queries, download sdf files
            classyfire_sdf_values, classy_sdf_downoaded = get_classyfire_results(
                cf_id, full_file, return_format, classyfire_search, classyfire_df
            )
            ...
            if classyfire_sdf_values and not row["direct_parent"]:
                ...
                if direct_parent:
                    pubchem_df.iloc[
                        rowIdx, get_idx("direct_parent", pubchem_df_headers)
                    ] = direct_parent  # direct_parent from ClassyFire

                if is_a:
                    pubchem_df.iloc[
                        rowIdx, get_idx("RELATIONSHIP", pubchem_df_headers)
                    ] = is_a

                add_classyfire_sdf_info(
                    mtbls_sdf_file_name,
                    relationships=is_a,
                    definition=definition,
                    name=name,
                    iupack_name=iupac_name,
                )
            ...
            if database_accession:
                ...
                if (
                    cactus_synonyms
                    and cactus_synonyms.strip(";") not in database_accession
                ):
            ...
            if not os.path.isfile(full_file):
                if "MTBLS" not in cid:
                    ...
                    try:
                        pcp.download(
                            "SDF", full_file, cid, overwrite=True
                        )  # try to pull down the sdf from PubChem
                    except Exception as e:
                        print_log(
                            "       -- Error: could not download SDF file for CID "
                            + cid
                            + ". "
                            + str(e),
                            mode="error",
...
def concatenate_sdf_files(pubchem_df, study_location, sdf_file_name, run_silently):
    print_log(
        "Concatenating the PubChem and ClassyFire SDF information into the ChEBI submission SDF"
    )
    ...
    with open(sdf_file_name, "w") as outfile:
        for idx, row in pubchem_df.iterrows():
            ...
            if (
                p_cid
                and not p_db_id.startswith("CHEBI:")
                and p_cid not in final_cid_list
            ):
                final_cid_list.append(p_cid)
                mtbls_sdf_file_name = os.path.join(
                    study_location, "mtbls_" + p_cid + pubchem_sdf_extension
                )
...
def removeHydrogenCheml(sdf_file_name):
    ...
    if fileSize <= 2000000:
        ...
        res = requests.post(
            "https://www.ebi.ac.uk/chembl/api/utils/removeHs", data=data
        )
        ...
        for mol in molecule:
            if count <= mol_count:
                ...
                if fileText[:1] == "\n":
                    fileText = fileText[1:]
                res = requests.post(
                    "https://www.ebi.ac.uk/chembl/api/utils/removeHs",
                    data=fileText.encode("utf-8"),
                )
                if res.status_code != 200:
                    print_log(
                        "removeHydrogen -- Error: could not remove Hydrogens"
                        + str(res.status_code),
                        mode="error",
                    )
                    break
...
"""
    try:
        if os.path.isfile(pubchem_sdf_file_name):
            ...
            print_log(
                "Error, can not read PubChem SDF file " + pubchem_sdf_file_name,
                mode="error",
            )

    except Exception as e:
        print_log(
            "    -- Could not read from PubChem SDF file: "
            + sdf_file_name
            + ". Error: "
            + str(e),
            mode="error",
...
def add_classyfire_sdf_info(
    mtbls_pubchem_sdf_file,
    mtbls_accession=None,
    relationships=None,
    name=None,
    definition=None,
    iupack_name=None,
    ...
    if os.path.isfile(mtbls_pubchem_sdf_file):
        with open(mtbls_pubchem_sdf_file, "r", encoding="utf-8") as infile:
            ...
            if database_accession:
                database_accession = ";".join(
                    unique_list(database_accession.split(";"))
                )
                filedata = filedata.replace(
                    "#template_database_accessions#", database_accession.rstrip(";")
                )
            ...
            if relationships:
                filedata = filedata.replace(
                    "#template_relationships#", relationships.rstrip(";")
                )
            ...
            if organism:
                ...
                if organism_part:
                    filedata = filedata.replace(
                        "#template_organism_part#", organism_part
                    )
                ...
                if mtbls_accession:
                    filedata = filedata.replace(
                        "#template_mtbls_accession#", mtbls_accession
                    )
...
def classyfire(inchi):
    ...
    try:
        r = requests.post(
            url + "/queries.json",
            data='{"label": "%s", "query_input": "%s", "query_type": "STRUCTURE"}'
            % (label, inchi),
...
def get_classyfire_results(
    query_id, classyfire_file_name, return_format, classyfire_search, classyfire_df
):
    all_ancestors = None
    ...
    if classyfire_search and query_id and query_id != "None":
        try:
            ...
            classyfire_file_name = classyfire_file_name.replace(
                "_pubchem.sdf", "_classyfire.sdf"
            )
            ...
            if not os.path.isfile(classyfire_file_name):
                ...
                start_time = time.time()
                print_log(
                    "       -- Getting ClassyFire SDF for query id: " + str(query_id)
                )
                url = get_settings().chebi.pipeline.search_services.classyfire_url
                r = requests.get(
                ...
                try:
                    if r and len(r.text) > 1:
                        text = r.text
                        with open(
                            classyfire_file_name, "w", encoding="utf-8"
                        ) as cf_file:
                            cf_file.write(text)
                            cf_file.close()
                            classy_sdf_downoaded = True
                        print_log(
                            "       -- ClassyFire SDF download took %s seconds"
                            % round(time.time() - start_time, 2)
                        )
                except Exception as e:
                    print_log(
                        "       -- ERROR: Could not read classyFire SDF file "
                        + classyfire_file_name
                        + ". Error:"
                        + str(e)
                    )
                ...
                print_log(
                    "       -- Already downloaded ClassyFire SDF for query id: "
                    + str(query_id)
                )
            ...
            print_log(
                "    -- ERROR: Could not get classyFire SDF for "
                + query_id
                + ". Error:"
                + str(e),
                mode="error",
    ...
    try:
        ...
        for idx, line in enumerate(lines):
            ...
            if inchi_key and parent_name and is_a:
                return {
                    "inchi_key": inchi_key.replace("InChiKey=", ""),
                    "direct_parent": parent_name,
                    "c_sdf_file": classyfire_file_name,
                    "is_a": is_a,
                }

    except Exception as e:
        print_log(
            "    -- Could not read from Classyfire SDF file: "
            + classyfire_file_name
            + ". Error: "
            + str(e)
        )
...
def get_classyfire_lookup_mapping():
    assay_master_template = os.path.join(
        resource_folder, "ClassyFire_Mapping_VLOOKUP.tsv"
    )
...
def get_chebi_client():
    ...
    while not client and retries - attempts >= 0:
        try:
            ...
            print_log(
                "      -- Trying to set up a new ChEBI webservice client in 5 minutes."
            )
            attempts += 1
            client = None
            time.sleep(
                wait_time
            )  # Wait as the resync of the ChEBI webservice in London can take up to 5 mins

    return client


def direct_chebi_search(
    final_inchi_key,
    comp_name,
    acid_chebi_id=None,
    search_type="inchi",
    ...
    try:
        if search_type == "inchi" and final_inchi_key:
            print_log(
                f" -- Querying ChEBI web services 2 based on final InChIKey {final_inchi_key}"
            )
            chebi_id = chebi_search_v2(search_term=final_inchi_key)
        elif search_type == "external_db" or search_type == "synonym":
            if comp_name:
                print_log(
                    f"    -- Querying ChEBI web services based on compound name {comp_name}"
                )
            ...
            if acid_chebi_id:
                chebi_ids = get_all_ontology_children_in_path(
                    acid_chebi_id=chebi_id, relation="is conjugate acid of"
                )
                if chebi_ids:
    ...
    if formula and charge and "-" in charge and not loop_counter:
        ...
        return direct_chebi_search(
            final_inchi_key,
            comp_name,
            acid_chebi_id=chebi_id,
            search_type="get_conjugate_acid",
            loop_counter=True,
        )
                ...
                if csid:
                    try:
                        ...
                        print_log(
                            "    -- Found CSID "
                            + csid
                            + " using ChemSpider, inchikey: "
                            + inchikey
                        )
                        return csid
                    except IndexError:
                        print_log(
                            "    -- Could not find CSID in ChemSpider, inchikey: "
                            + inchikey
                        )
                        return ""
    except Exception as e:
...
def check_chebi_api(search_term="BSYNRYMUTXBXSQ-UHFFFAOYSA-N"):
    status = "Failure"
...
def add_database_name_synonym(synonym):
    if synonym.startswith("LM"):  # LipidMaps
        ...
        synonym.startswith("D")
        and synonym[1:2] != "-"
        and is_correct_int(synonym[1:], 5)
    ):  # KEGG Drug
            ...
            if not pubchem_compound and search_category == "substance":
                ...
                if _cid:
                    ...
                    compound = pubchem_compound[
                        0
                    ]  # Only read the first record from PubChem = preferred entry
        ...
        if compound:
            ...
            for idx, synonym in enumerate(compound.synonyms):
                ...
                if idx == 0 and synonym.lower() != comp_name.lower():
                    ...
                    idx == 1
                    and first_synonym is None
                    and synonym.lower() != comp_name.lower()
                ):
...
def get_sdf(
    anno_sub_folder_path, cid, iupac, sdf_file_list, final_inchi, classyfire_search
):
    classyfire_id = ""
    file_name = ""
    ...
    if anno_sub_folder_path and cid:
        ...
        if cid.endswith(".mol"):  # We have added a manually created/downloaded mol file
            ...
            if os.path.isfile(mol_file_name):
                ...
                print_log(
                    "    -- ERROR: Could not find manually created MOl structure "
                    + mol_file_name,
                    mode="error",
                )
        else:
            ...
            if os.path.isfile(full_file):
                print_log(
                    "    -- Already have PubChem SDF for CID "
                    + str(cid)
                    + " for name: "
                    + iupac
                )
                ...
                if "MTBLS" not in cid:
                    print_log(
                        "    -- Getting SDF for CID " + str(cid) + " for name: " + iupac
                    )
...
def processUniChemResponse(inchi_key):
    ...
    try:
        ...
        if resp.status_code == 200:
            json_resp = resp.json()
            response_dict = {
                item["src_id"]: item for item in json_resp if item and "src_id" in item
            }
            if len(response_dict) == 0:
                print_log(
                    " Chem plus search for inchi key - "
                    + inchi_key
                    + " contains no data."
                )
                return unichem_id
            ...
            if "3" in response_dict:
                unichem_id = (
                    unichem_id
                    + "PDBeChem:"
                    + response_dict["3"]["src_compound_id"]
                    + ";"
                )
            ...
            if "18" in response_dict:
                unichem_id = (
                    unichem_id
                    + "HMDB:HMDB"
                    + response_dict["18"]["src_compound_id"][4:]
                    + ";"
                )
            if "25" in response_dict:
                unichem_id = (
                    unichem_id + "LINCS:" + response_dict["25"]["src_compound_id"] + ";"
                )
            if "34" in response_dict:
                unichem_id = (
                    unichem_id
                    + "DrugCentral:"
                    + response_dict["34"]["src_compound_id"]
                    + ";"
                )
            unichem_id = unichem_id.rstrip(";")
            return unichem_id
        ...
        if resp.status_code == 200:
            try:
                ...
                if "PubChem" in response_dict and response_dict["PubChem"]:
                    dime_db_ids = (
                        dime_db_ids + "PubChem:" + response_dict["PubChem"] + ";"
                    )
                if "KEGG Compound" in response_dict and response_dict["KEGG Compound"]:
                    dime_db_ids = (
                        dime_db_ids
                        + "KEGG COMPOUND:"
                        + response_dict["KEGG Compound"]
                        + ";"
                    )
                ...
                logger.warning(
                    f"Invalid result from {dime_url} for input: {inchi_key}  {str(ex)}"
                )
    except Exception as e:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 6 and sys.argv[6]:
        classyfire_supdate_study_mafearch = (
            True if sys.argv[5].lower() == "true" else False
        )
