...
def serialize_comment(isa_obj):
    ...
    return {"name": isa_obj.name, "value": isa_obj.value}
...
def unserialize_comment(json_obj):
    ...
    return Comment(name=name, value=value)
...
def serialize_ontology_source(isa_obj):
    ...
    return {
        ...
        "comments": json.loads(
            json.dumps(isa_obj.comments, default=serialize_comment, sort_keys=True)
        ),
    }
...
def serialize_protocol(isa_obj):
    ...
    return {
        ...
        "protocolType": json.loads(
            json.dumps(
                isa_obj.protocol_type,
                default=serialize_ontology_annotation,
                sort_keys=True,
            )
        ),
        "description": isa_obj.description,
        ...
        "parameters": json.loads(
            json.dumps(
                isa_obj.parameters, default=serialize_protocol_parameter, sort_keys=True
            )
        ),
...
def serialize_person(isa_obj):
    ...
    return {
        ...
        "roles": json.loads(
            json.dumps(
                isa_obj.roles, default=serialize_ontology_annotation, sort_keys=True
            )
        ),
        "comments": json.loads(
            json.dumps(isa_obj.comments, default=serialize_comment, sort_keys=True)
...
def serialize_study_publication(isa_obj):
    ...
    return {
        ...
        "status": json.loads(
            json.dumps(
                isa_obj.status, default=serialize_ontology_annotation, sort_keys=True
            )
        ),
        "comments": json.loads(
            json.dumps(isa_obj.comments, default=serialize_comment, sort_keys=True)
...
def serialize_characteristic(isa_obj):
    ...
    return {
        "category": json.loads(
            json.dumps(
                isa_obj.category, default=serialize_ontology_annotation, sort_keys=True
            )
        ),
        "value": json.loads(
            json.dumps(
                isa_obj.value, default=serialize_ontology_annotation, sort_keys=True
            )
        ),
        "unit": json.loads(
            json.dumps(
                isa_obj.unit, default=serialize_ontology_annotation, sort_keys=True
            )
        ),
        "comments": json.loads(
            json.dumps(isa_obj.comments, default=serialize_comment, sort_keys=True)
...
def unserialize_characteristic(json_obj):
    ...
    return Characteristic(category=category, value=value, unit=unit, comments=comments)
...
def serialize_factor_value(isa_obj):
    ...
    return {
        "factorName": json.loads(
            json.dumps(
                isa_obj.factor_name, default=serialize_study_factor, sort_keys=True
            )
        ),
        "value": json.loads(
            json.dumps(
...
def unserialize_factor_value(json_obj):
    ...
    return FactorValue(
        factor_name=factor_name, value=value, unit=unit, comments=comments
    )
...
def serialize_study_source(isa_obj):
    ...
    return {
        ...
        "characteristics": json.loads(
            json.dumps(
                isa_obj.characteristics,
                default=serialize_characteristic,
                sort_keys=True,
            )
        ),
        "comments": json.loads(
...
def unserialize_study_source(json_obj):
    ...
    return Source(name=name, characteristics=characteristics, comments=comments)
...
def serialize_study_sample(isa_obj):
    ...
    return {
        ...
        "factorValues": json.loads(
            json.dumps(
                isa_obj.factor_values, default=serialize_factor_value, sort_keys=True
            )
        ),
        "derives_from": json.loads(
            json.dumps(
                isa_obj.derives_from, default=serialize_study_source, sort_keys=True
            )
        ),
        "comments": json.loads(
            json.dumps(isa_obj.comments, default=serialize_comment, sort_keys=True)
...
Study_api_model = {
    ...
    "studyDesignDescriptors": fields.List(
        fields.Nested(Inv_OntologyAnnotation_api_model)
    ),
...
def serialize_investigation(isa_obj):
    ...
    return {
        ...
        "ontologySourceReferences": json.loads(
            json.dumps(
                isa_obj.ontology_source_references,
                default=serialize_characteristic,
                sort_keys=True,
            )
        ),
