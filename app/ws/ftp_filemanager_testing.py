...
class FTPRemoteFileManager(Resource):
    @swagger.operation(
        ...
        parameters=[
            ...
            },
            {
                ...
                "enum": [
                    ...
                    "delete ftp folder",
                    "check folder exists",
                    "check ftp folder status",
                    "check sync status",
                    "sync folder from ftp",
                    "sync folder from study",
                    "sync to public ftp",
                    "get ftp folder permission",
        ...
        responseMessages=[
            ...
            {"code": 403, "message": "Forbidden. Please provide a valid user token"},
            {
                "code": 404,
    ...
    def post(self):
        ...
        if operation == "create ftp folder":
            try:
                ...
                data_mover_storage: DataMoverAvailableStorage = (
                    DataMoverAvailableStorage("ftp_filemanager_testing", study_id, app)
                )
            ...
            try:
                ...
                status = data_mover_storage.move_ftp_folder(
            ...
            try:
                ...
                output = data_mover_storage.update_ftp_folder_permission(
                    source_folder, chmod
                )
                if output:
            ...
            try:
                ...
                meta_calc_result, rdfiles_calc_result = (
                    data_mover_storage.sync_anaysis_job_results(
                        source_ftp_folder=source_folder, force=False
                    )
                )
                result = {
                    "result": {
                        "meta_calc_result": meta_calc_result.model_dump(),
                        "rdfiles_calc_result": rdfiles_calc_result.model_dump(),
                    }
                }
            except Exception as e:
                logger.info(e)
            ...
            try:
                ...
                sync_metafiles_result, sync_rdfiles_result = (
                    data_mover_storage.get_folder_sync_results()
                )
                result = {
                    "result": {
                        "sync_metafiles_result": sync_metafiles_result.model_dump(),
                        "sync_rdfiles_result": sync_rdfiles_result.model_dump(),
                    }
                }
            except Exception as e:
            ...
            try:
                ...
                meta_sync_status, rdfiles_sync_status = (
                    data_mover_storage.sync_from_ftp_folder(source_folder)
                )
                result = {
                    "meta_sync_status": meta_sync_status,
                    "rdfiles_sync_status": rdfiles_sync_status,
                }
            except Exception as e:
                logger.info(e)
                print(e)
            ...
            try:
                ...
                meta_sync_status, files_sync_status, chebi_sync_status = (
                    data_mover_storage.sync_from_studies_folder(target_folder)
                )
                result = {
                    "meta_sync_status": meta_sync_status,
                    "files_sync_status": files_sync_status,
                    "chebi_sync_status": chebi_sync_status,
                }
            except Exception as e:
                logger.info(e)
                print(e)
            ...
            try:
                ...
                meta_public_sync_status, files_public_sync_status = (
                    data_mover_storage.sync_public_study_to_ftp()
                )
                result = {
