...
def convert_relative_to_real_path(relative_path: str) -> str:
    ...
    removed_prefix = (
        relative_path.replace(f".{os.sep}", "", 1)
...
def get_all_metadata_files(study_metadata_files_path: Union[None, str] = None):
    ...
    for pattern in patterns:
        metadata_files.extend(
            glob.glob(os.path.join(study_metadata_files_path, pattern), recursive=False)
        )
    return metadata_files


def find_study_data_files(
    study_metadata_path,
    filtered_subfolder=None,
    search_pattern=None,
    match_files=True,
    match_folders=True,
):
    ...
    for desc in source_file_descriptors:
        if match_files and not source_file_descriptors[desc].is_dir:
            search_results.append(
                f"{filtered_subfolder}/{desc}" if filtered_subfolder else desc
            )
...
def get_all_study_metadata_and_data_files(
    ...
    if os.path.exists(study_metadata_path):
        source_folders_iter = get_study_folder_files(
            ...
            add_sub_folders=add_sub_folders,
        ...
        if item.is_dir():
                ...
                if files:
                    ...
                    for parameters_file in SKIP_FOLDER_CONTAINS_ANY:
                        ...
                        expected_files = [
                            ...
                            and "_" not in os.path.basename(x)
                        ]
                        if expected_files:
                            referenced_sub_files = referenced_sub_files.union(
                                set(expected_files)
                            )
            ...
            if ext in STOP_FOLDER_EXTENSIONS or is_stop_folder:
                ...
                if is_stop_folder:
                    for subfile in referenced_sub_files:
                        relative_path = (
                            str(subfile).replace(f"{root_path}", "").lstrip("/")
                        )
                        file_descriptors[relative_path] = FileDescriptor(
                            relative_path=relative_path,
                            is_dir=False,
                ...
                if recursive:
                    yield from get_study_folder_files(
                        root_path,
                        file_descriptors,
                        item,
                        list_all_files=list_all_files,
            ...
            if not list_all_files:
                ...
                if not include_metadata_files:
                    if (
                        len(item.name) > 2
                        and item.name[:2] in METADATA_FILE_PREFIXES
                        and (item.name.endswith(".txt") or item.name.endswith(".tsv"))
                    ):
                        continue
