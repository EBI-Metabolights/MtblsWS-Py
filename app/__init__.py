...
if build_number_file.exists():
    __build_number__ = (
        build_number_file.read_text(encoding="utf-8").replace("\n", "").strip()
    )
...
if app_version_file.exists():
    app_version_number = (
        app_version_file.read_text(encoding="utf-8").replace("\n", "").strip()
    )

__app_version__ = (
    app_version_number + f"-{__build_number__}"
    if __build_number__
    else app_version_number
)
...
__api_version__ = (
    api_version_number + f"-{__build_number__}"
    if __build_number__
    else api_version_number
)
