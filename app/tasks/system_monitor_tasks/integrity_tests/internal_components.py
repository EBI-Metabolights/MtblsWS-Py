...
from app.tasks.common_tasks.basic_tasks.execute_commands import (
    execute_bash_command as common_worker_execute,
)
...
def check_redis():
    ...
    if actual != value:
        raise Exception(
            f"Result contains unexpected value: Actual: {actual} Expected: {value}"
        )
    return actual
