...
options = {"server": "https://www.ebi.ac.uk/panda/jira/"}
...
@celery.task(
    ...
    name="app.tasks.common_tasks.admin_tasks.create_jira_tickets.update_or_create_jira_issue_task",
)
def update_or_create_jira_issue_task(user_token: str):
    ...
    try:
        ...
        try:
            ...
            return (
                False,
                "Could not connect to JIRA server, incorrect username or password?",
                updated_studies,
            )
        ...
        for study in studies:
            ...
            issues = jira.search_issues(
                search_param
            )  # project = MetaboLights AND summary ~ 'MTBLS121 '
            new_summary = (
                study_id
                + " - "
                ...
                if issues:
                    ...
                    if study_status == "Provisional" or study_status == "Private":
                        ...
                        issue = jira.create_issue(
                            project=mtbls_project.key,
                            summary="MTBLS study - To be updated",
            ...
            )  # Follow pattern 'MTBLS123 - YYYYMMDD - Status'
            ...
            if curator:
                if curator.lower() == "mark":
                    ...
                    curator.lower() == "xuefei"
                    or curator.lower() == "reza"
                    or curator.lower() == "keeva"
                ):
            ...
            curator_update = (
                True
                if assignee != default_curator and jira_curator != default_curator
                else False
            )
            if assignee_changed or summary_changed:
                ...
                jira.add_issues_to_epic(
                    curation_epic, issues_to_add
                )  # Add the Curation Epic
                ...
                if jira_curator == default_curator:
                    comment_text = (
                        comment_text
                        + '. Default curator has been changed from "'
                        + curator
                        + '" to "'
                        + default_curator
                        + '"'
                ...
                issue.update(
                    summary=new_summary, fields={"labels": labels}, notify=False
                )
        ...
        send_email(
            "Jira Tickets creation Task", "Ticket(s) update failed", None, email, None
        )
        return False, "Update failed: " + str(e), str(study_id)
    send_email(
        "Jira Tickets creation Task",
...
def maintain_jira_labels(issue, study_status, user_name):
    ...
    if (
        not curation_flag
    ):  # Do not confuse with the In Curation status, this is a "curator tasks" flag
        labels.append(curation_label)

    if (
        study_status == "Provisional" and not submitted_flag
    ):  # The "Submitted" label is not present
