...
def sync_compound_on_es_and_db(email: str, send_email_to_submitter=False):
    try:
        ...
        with DBManager.get_instance().session_maker() as db_session:
            ...
            if not metabolites:
                raise MetabolightsDBException("There are no metabolite on database")
                        ...
                        if compound_id not in compounds_dict:
                            ...
                            if updated_date:
                                if compounds_dict[compound_id]:
                                    item_update_date = compounds_dict[
                                        compound_id
                                    ].strftime("%Y-%m-%d")
        ...
        status = (
            "UPDATED"
            if out_of_date_compounds or unindexed_compounds or compounds_not_in_db
            else "NO CHANGE"
        )
        result = {
        ...
        if send_email_to_submitter:
            ...
            send_email(
                "Result of the task: sync MetaboLights compounds on elasticsearch and database",
                result_str,
                None,
                email,
                None,
