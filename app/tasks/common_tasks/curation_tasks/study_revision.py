...
@celery.task(bind=True, base=MetabolightsTask, default_retry_delay=10, max_retries=3, soft_time_limit=60*15, name="app.tasks.common_tasks.curation_tasks.study_revision.prepare_study_revision")
    ...
    with DBManager.get_instance().session_maker() as db_session:
        try:
            ...
            study_revision: StudyRevision = query.filter(StudyRevision.accession_number == study_id, 
                                                        StudyRevision.revision_number == study.revision_number).first()
            if not study_revision:
                raise Exception(f"Study revision not found for study {study_id} revision {study.revision_number}")
...
@celery.task(bind=True, base=MetabolightsTask, default_retry_delay=10, max_retries=3, soft_time_limit=60*15, name="app.tasks.common_tasks.curation_tasks.study_revision.delete_study_revision")
def delete_study_revision(self, study_id: str, revision_number: int):
    
    revision = StudyRevisionService.delete_study_revision(study_id, int(revision_number))
    
    return {"study_id": study_id, "deleted-revision": revision.model_dump()}



@celery.task(bind=True, base=MetabolightsTask, default_retry_delay=10, max_retries=3, soft_time_limit=60*15, name="app.tasks.common_tasks.curation_tasks.study_revision.sync_study_metadata_folder")
...
def sync_public_ftp_folder_with_metadata_folder(task_name: str, 
                           study_id: str, 
                           email: str, 
                           source_path: str, 
                           target_path: str, 
...
def sync_public_ftp_folder_with_revisions(task_name: str, 
                           study_id: str, 
                           revision_number: int,
                           email: str, 
                           source_path: str, 
