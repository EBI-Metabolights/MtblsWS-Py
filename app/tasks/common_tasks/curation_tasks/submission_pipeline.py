...
def validate_study_task(self, params: dict[str, Any]):
    try:
        ...
        token = auth_manager.create_oauth2_token_by_api_token(user_token)
        validation_endpoint = "https://www6.ebi.ac.uk/metabolights/ws3/validations"
        url = f"{validation_endpoint}/{model.study_id}"
        ...
        if not task_id:
            raise StudySubmissionError(f"Study validation does not start. {str(response_json)}")
        sleep_period = 10
        max_retry = 60
        retry = 0
        poll_url = f"{validation_endpoint}/{model.study_id}/result"
        ...
        if validation_status is None:
            raise StudySubmissionError(f"Study validation status polling failed. {str(latest_response)}")

        if validation_status in {"error"}:
            if not model.test:
                raise StudySubmissionError(f"Study validation failed. {str(latest_response)}")
