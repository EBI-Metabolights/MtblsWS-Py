    ...
    try:        
        ...
        with flask_app.app_context():
            ...
            with DBManager.get_instance(flask_app).session_maker() as db_session:
                ...
                result = db_session.query(Study.acc, Study.updatedate).filter(Study.status == StudyStatus.PUBLIC.value).all()
            ...
            result = reindex_studies_in_list(user_token, flask_app, studies)
...
def reindex_studies_in_list(user_token, flask_app, studies):
