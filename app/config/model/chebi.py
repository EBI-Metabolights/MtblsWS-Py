...
class ChebiPipelineSearchServices(BaseModel):
    ...
    opsin_url: str = "https://www.ebi.ac.uk/opsin/ws/"
    ...
    dime_url: str = (
        "https://dimedb.ibers.aber.ac.uk/api/metabolites"
        "?where=%7B%22_id%22%20:%20%22INCHI_KEY%22%7D&projection=%7B%22External%20Sources%22%20:%201%7D"
    )
...
class ChebiPipelineConfiguration(BaseModel):
    ...
    curated_metabolite_list_file_location: str = " ./resources/chebi_masterlist.txt"
    chebi_upload_script: str
    chebi_pipeline_url: str
    chebi_pipeline_annotation_folder: str = "chebi_pipeline_annotations"
    obo_file: str
    chebi_url_wait: int = 300
    removed_hs_mol_count: int = 500
    classyfire_mapping: str
    search_services: ChebiPipelineSearchServices = ChebiPipelineSearchServices()


class ChebiServiceSettings(BaseModel):
    connection: ChebiServiceConnection = ChebiServiceConnection()
    configuration: ChebiServiceConfiguration = ChebiServiceConfiguration()
