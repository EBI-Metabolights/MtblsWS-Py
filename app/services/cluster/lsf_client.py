...
class LsfClient(HpcClient):
    ...
    def submit_hpc_job(
        ...
        bsub_command = self._get_submit_command(
            script_path,
            queue,
            job_name,
            output_file,
        ...
        if match:
            ...
            raise MetabolightsException(
                message=f"output does not have a job id. {str(result.stdout)}\n {str(result.stderr)}"
            )
        ...
        for line in lines:
            if line.strip():
                ...
                if match:
                    ...
                    if (
                        not match_result[0]
                        or not match_result[0].isnumeric()
                        and not failing_gracefully
                    ):
                        raise MetabolightsException(message="No job id is defined.")
    ...
    def get_job_status(
        ...
        if result.stdout:
            ...
            for line in lines:
                if line.strip():
                    ...
                    try:
                        ...
                        submit_datetime = submit_datetime.replace(
            ...
            raise MetabolightsException(message="No result returned.")
    ...
    def _prepare_script_to_submit_on_hpc(
        ...
        if runtime_limit:
            ...
            bsub_comments.append(
                f"-W {self.convert_to_runtime_limit(self.settings.job_default_runtime_limit_in_secs)}"
            )
        if mem:
            bsub_comments.append(f"-R {mem}")
        else:
            bsub_comments.append(
                f"-R rusage[mem={self.settings.job_default_memory_in_mb}MB]"
            )
        if cpu > 1:
