...
class SlurmClient(HpcClient):
    ...
    def submit_hpc_job(self, script_path: str, job_name: str, output_file=None, error_file=None, account=None, queue: Union[None, str] = None, timeout: Union[None, float]=30.0, runtime_limit: Union[None, str] = None, cpu: int=1, mem: str="") -> SubmittedJobResult:
        ...
        hpc_command = self._get_submit_command(script_path=script_path, 
                                               queue=queue, 
                                               job_name=job_name, 
    ...
    def kill_jobs(self, job_id_list: List[str], failing_gracefully=False, timeout: Union[None, float]=30.0) -> SubmittedJobResult:
        ...
        if result.returncode == 0:
            return SubmittedJobResult(return_code=result.returncode, job_ids=job_id_list, stdout=result.stdout if result.stdout else [], stderr=result.stderr if result.stderr else [])

        if not failing_gracefully:
            raise MetabolightsException(message=f"No job id is defined.")
        return SubmittedJobResult(return_code=result.returncode, job_ids=[], stdout=result.stdout if result.stdout else [], stderr=result.stderr if result.stderr else [])
        ...
        if result and result.stdout:
            ...
            for line in lines:
                if line.strip():
                    ...
                    job_name = ""
                    if not columns[6].strip().startswith(f"{self.settings.job_prefix}{delimeter}"):
                        continue
                    else:
                        job_name = columns[6].strip().replace(f"{self.settings.job_prefix}{delimeter}", "")
                    status = columns[2]
                    job_id = columns[0]
                    partition = columns[1]
                    ...
                    try:
                        ...
                        pass
                    item = HpcJob(job_id=job_id, status=status, name=columns[6].strip(), submit_time=submit_time, queue=partition)
                    if  job_names:
    ...
    def _prepare_script_to_submit_on_hpc(self, script_path: str, job_name: str, queue=None, output_file=None, error_file=None, account=None, runtime_limit: Union[None, str] = None, cpu:int = 1, mem: str = ""):
        ...
        hpc_comments.append(f"-J {self.settings.job_prefix}{self.settings.job_prefix_demimeter}{job_name}")
        if queue:
        ...
        if mem:
            ...
            hpc_comments.append(f"--mem={str(self.settings.job_default_memory_in_mb)}MB")
