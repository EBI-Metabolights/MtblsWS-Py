...
from app.services.storage_service.unmounted.unmounted_file_manager import (
    UnmountedVolumeFileManager,
)
...
class UnmountedStorage(Storage):
    def __init__(self, name, app=None, remote_file_manager=None):
        ...
        if not remote_file_manager:
            self.remote_file_manager: UnmountedVolumeFileManager = (
                UnmountedVolumeFileManager(manager_name, app)
            )

        super(UnmountedStorage, self).__init__(
    ...
    def sync_from_local(
        ...
        **kwargs,
        ...
        remote_job_manager = DataMoverAvailableStorage(
            "unmounted_storage", study_id, self.app
        )
        meta_sync_status, files_sync_status, chebi_sync_status = (
            remote_job_manager.sync_from_studies_folder(
                target_folder, ignore_list, **kwargs
            )
        )
    ...
    def sync_to_public_ftp(
        ...
        **kwargs,
        ...
        meta_public_sync_status, files_public_sync_status = (
            remote_job_manager.sync_public_study_to_ftp(
                source_study_folder=source_local_folder,
                target_ftp_folder=target_folder,
    ...
    def sync_from_storage(
        ...
        **kwargs,
        ...
        if not ignore_list:
            ignore_list = []
        meta_sync_status, rdfiles_sync_status = remote_job_manager.sync_from_ftp_folder(
            source, ignore_list, **kwargs
        )
    ...
    def calculate_sync_status(
        ...
        meta_calc_result, rdfiles_calc_result = (
            remote_job_manager.sync_anaysis_job_results(
                source_ftp_folder=ftp_folder_name, force=force, ignore_list=ignore_list
            )
        )
    ...
    def check_folder_sync_status(
        ...
        sync_metafiles_result, sync_rdfiles_result = (
            remote_job_manager.get_folder_sync_results()
        )
