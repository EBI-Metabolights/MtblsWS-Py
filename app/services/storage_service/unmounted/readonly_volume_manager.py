...
class ReadOnlyFileVolumeManager(object):
    ...
    def create_folder(
        ...
        try:
            result = self.create_folder_on_remote_volume(
                study_id, "readonly_volume_create_folder", paths, acl.value, exist_ok
            )
    ...
    def create_folder_on_remote_volume(
        self,
        study_id: str,
        requestor: str,
        folder_paths: Union[str, List[str]],
        ...
        if paths:
            ...
            output: CommandOutput = self._execute_and_get_result(
                study_id, requestor, command, params
            )
    ...
    def _execute_and_get_result(
        self, study_id: str, requestor: str, command: str, params: str
    ) -> CommandOutput:
        mounted_paths = get_settings().hpc_cluster.datamover.mounted_paths
        study_internal_files_folder = os.path.join(
            get_settings().study.mounted_paths.study_internal_files_root_path, study_id
        )
        study_log_folder = os.path.join(
            study_internal_files_folder, self.study_settings.internal_logs_folder_name
        )
        cluster_study_internal_files_folder = os.path.join(
            mounted_paths.cluster_study_internal_files_root_path, study_id
        )
        cluster_study_log_folder = os.path.join(
            cluster_study_internal_files_folder,
        ...
        status, message, job_out, job_err, log_file = submit_job(
            False,
            None,
            queue=get_settings().hpc_cluster.datamover.default_queue,
            job_cmd=command,
            job_params=params,
            submitter=requestor,
            log=True,
        ...
        status1 = self.check_if_job_successful(
            status=status, job_out=job_out, log_file_study_path=log_file_study_path
        )
        self._log_job_output(
            status=status,
            job_out=job_out,
            job_err=job_err,
            log_file_study_path=log_file_study_path,
        )
        ...
        if status:
            if "is submitted to queue" in job_out:
                for x in range(0, self.cluster_settings.job_status_read_timeout):
                    ...
                    time.sleep(1)
                logger.error(
                    f"Failed to read the file content in {self.cluster_settings.job_status_read_timeout} seconds"
                )
                return False
            else:
