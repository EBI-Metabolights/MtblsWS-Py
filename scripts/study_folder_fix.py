...
if __name__ == "__main__":
    ...
    for study_id in study_ids:
            ...
            backup_folders = glob.glob(f"{audit_folder_path}/{audit_folder_prefix}*{audit_folder_suffix}")
            backup_folder = None
            if backup_folders:
                backup_folder = backup_folders[0]    
        
            internal_files_path = maintenance_task.study_internal_files_path
            log_files = glob.glob(f"{internal_files_path}/logs/{study_id}_{log_folder_prefix}*/*{log_folder_suffix}")
            assay_files = set()
                ...
                if os.path.exists(target_file) and backup_folder:
                    ...
                    for item in files:
                        ...
                        if basename == target_file_base_name:
                            ...
                            updated_file_name = maintenance_task.sanitise_metadata_filename(study_id, basename, prefix="a_")
                    ...
                    if backup_file:
                        ...
                        if len(backup_file_pd) == len(target_file_pd):
                            for index, row in target_file_pd.iterrows():
                                ...
                                polarization: str = backup_file_pd.iloc[index]["Parameter Value[Scan polarity]"]
                                target_sample_name_base = target_file_pd.iloc[index]["Sample Name"]
                                target_sample_name = unidecode(target_sample_name_base).strip().replace(" ","")
                                src_sample_name = unidecode(sample_name).strip().replace(" ","")
                                if src_sample_name == target_sample_name:
                                    polarization = backup_file_pd.iloc[index]["Parameter Value[Scan polarity]"]
                                    new_value = None
                                    if polarization:
                                        if "pos" in polarization.lower() and ("neg" in polarization.lower() or "nagetive" in polarization.lower()):
                                            ...
                                            new_value = "positive"
                                        elif "neg" in polarization.lower() or "nagative" in polarization.lower():
                                            new_value = "negative"
                                            ...
                                            new_value = "alternating"
                                        if not new_value:
                                            print(f"{study_status.name} {study_id}, {target_file_base_name} Invalid value '{polarization}' at {index + 1} ")
                                        else:
                                            current_value = target_file_pd.iloc[index]["Parameter Value[Scan polarity]"]
                                            if current_value != new_value:
                                                if study_status in [StudyStatus.INREVIEW, StudyStatus.PUBLIC]:
                                                    target_file_pd.iloc[index]["Parameter Value[Scan polarity]"] = new_value
                                                    # print(f"{study_status.name} {study_id}, {target_file_base_name} row: {index + 1}:  {current_value} -> {new_value} original: {polarization}")
                                                else:
                                                    target_file_pd.iloc[index]["Parameter Value[Scan polarity]"] = polarization
                                                    # print(f"{study_status.name} {study_id}, {target_file_base_name} row: {index + 1}:  keep old value: {polarization}")
                                                assay_updated = True
                                    else:
                                        if study_status in [StudyStatus.INREVIEW, StudyStatus.PUBLIC]:
                                            print(f"{study_id}, {target_file_base_name} Empty value at {index + 1} ")
                                        
                                else:
                                    not_sync = True
                                    print(f"! {study_id}, {target_file_base_name} row: {index + 1}: Not sync  old: {sample_name} new: {target_sample_name}")
                        else:
                            not_sync = True
                            print(f"! {study_id}, {target_file_base_name} row: {index + 1}: Not same column counts")
                    else:
                        print(f"! {study_id}, {target_file_base_name} Target file does not exist")
                if assay_updated:
