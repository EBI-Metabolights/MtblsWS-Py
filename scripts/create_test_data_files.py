...
def delete_test_file(study_id, file_path, item: FileDescriptor):
    if not os.path.exists(file_path):
        return
    print(f"{study_id}\tREMOVE\t{'FOLDER' if item.is_dir else 'FILE'}\t{item.extension}\t{item.relative_path}")
...
def create_dummy_file(study_id, file_path, item: FileDescriptor, unique_content=True):
    ...
    print(f"{study_id}\tCREATE\t{'FOLDER' if item.is_dir else 'FILE'}\t{item.extension}\t{item.relative_path}")


def create_dummy_zip_file(study_id, file_path, item: FileDescriptor, unique_content=True):
    if os.path.exists(file_path):
        return
        ...
        if item.is_stop_folder and item.sub_filename:
            if os.path.exists(item_path):
                ...
                if not items:
                    ...
                    create_dummy_file(study_id, os.path.join(study_test_path, sub_item.relative_path), sub_item)
                    return
                else:
                    if len(items) == 1 and items[0] == item.sub_filename:
                        os.utime(item_path, (item.modified_time, item.modified_time))
                        os.utime(os.path.join(item_path, item.sub_filename), (item.modified_time, item.modified_time))
                        return
                    else:
...
def create_study_test_data_files(
    ...
    for study_id in study_id_list:
        ...
        target_folders_iter = get_study_folder_files(
            study_source_path, target_file_descriptors, study_source_path_item, list_all_files=True
        )
...
if __name__ == "__main__":
    ...
    with flask_app.app_context():
        ...
        results = create_study_test_data_files(study_ids, source_path, target_path, flask_app=flask_app)
    print("end")
