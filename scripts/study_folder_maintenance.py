...
def are_metadata_files_sync(
    maintenance_task: StudyFolderMaintenanceTask, check_ftp_folders: bool = True
):
    ...
    private_ftp_path = os.path.join(
        mounted_paths.cluster_private_ftp_root_path, private_ftp_folder_name
    )

    legacy_study_path = os.path.join(
        mounted_paths.cluster_legacy_study_files_root_path, study_id
    )
    audit_path = os.path.join(
        mounted_paths.cluster_study_audit_files_root_path,
        study_id,
        study_settings.audit_folder_name,
    )
    ...
    if os.path.exists(audit_path):
        ...
        saved_private_ftp_path_hash = maintenance_task.read_hash_file(
            metadata_files_signature_root_path=saved_private_ftp_path,
        ...
        saved_audit_path_hash = maintenance_task.read_hash_file(
            metadata_files_signature_root_path=saved_audit_path,
            hash_file_name=hash_file_name,
        )
    ...
    legacy_study_path_hash, _ = maintenance_task.calculate_metadata_files_hash(
        search_path=legacy_study_path
    )
    print(f"Calculated study folder metadata hash: {legacy_study_path_hash}")
    study_folder_sync = (
        True if saved_audit_path_hash == legacy_study_path_hash else False
    )
    print(
        f"{study_id}: Study folder sync status: {study_folder_sync}, Folder exists: {True if saved_private_ftp_path else False}"
    )

    if check_ftp_folders:
        ftp_path_sync = False
        if saved_private_ftp_path:
            private_ftp_path_hash, _ = maintenance_task.calculate_metadata_files_hash(
                search_path=private_ftp_path
            )
            print(f"Calculated FTP folder metadata hash: {private_ftp_path_hash}")
            ftp_path_sync = (
                True if private_ftp_path_hash == saved_private_ftp_path_hash else False
            )
        else:
            private_ftp_metadata_files = get_files(
                private_ftp_path, patterns=metadata_file_patterns
            )
            print(
                f"There is no FTP folder backup folder. Number of metadata files on FTP folder: {len(private_ftp_metadata_files)}"
            )
            ftp_path_sync = True if not private_ftp_metadata_files else False

            print(
                f"{study_id}: FTP sync status: {ftp_path_sync}, Study folder sync status: {study_folder_sync}, Folder exists: {True if saved_private_ftp_path else False}"
            )
...
def maintain_folders(
    ...
    if not target:
        raise MaintenanceException(
            message="target should be 'metadata', 'data', 'private-ftp or combination of them with , char. Examples: 'metadata,data', 'metadata,data,private-ftp'"
        )
    ...
    with open(future_action_log_file_path, "w") as fa:
        ...
        for study_id in study_id_list:
            ...
            mounted_paths = get_settings().hpc_cluster.datamover.mounted_paths
            audit_folder_root_path = os.path.join(
                mounted_paths.cluster_study_audit_files_root_path,
                study_id,
                study_settings.audit_folder_name,
            )
            released_version_path = os.path.join(
                audit_folder_root_path, released_version
            )
            ...
            try:
                ...
                new_metadata_folder_exists = (
                    True if os.path.exists(new_metadata_path) else False
                )
                ...
                audit_path = os.path.join(
                    audit_folder_root_path, f"{maintenance_task.task_name}_BACKUP"
                )
                if not new_metadata_folder_exists:
                    maintenance_task.create_audit_folder(
                        metadata_files_path=legacy_study_path,
                    ...
                    folders_are_sync = are_metadata_files_sync(
                        maintenance_task, check_ftp_folders
                    )
                    if not skip_if_metadata_is_sync:
                        ...
                        for file in files:
                            maintenance_task.backup_file(
                                file,
                                reason="Replaced from legacy storage",
                                keep_file_on_folder=False,
                            )
                        include_list = ["[asi]_*.txt", "m_*.tsv"]
                        exclude_list = ["*"]
                        command = HpcRsyncWorker.build_rsync_command(
                        ...
                        if folders_are_sync:
                            print(
                                f"{study_id} metadata folder is sync. Skip this folder"
                            )
                            continue
                        else:
                            maintenance_task.create_audit_folder()
                ...
                if (
                    ...
                    and not os.path.exists(released_version_path)
                    and os.path.exists(legacy_study_path)
                ):
                    ...
                    maintenance_task.create_audit_folder(
                        ...
                        folder_name=released_version,
                        stage=None,
                    )

                if cluster_mode and "private-ftp" in target_list:
                    maintenance_task.backup_study_private_ftp_metadata_files = (
                        False if folders_are_sync else True
                    )
                    try:
                        ...
                        print(
                            f"Maintain task could not be completed for {study_id}. {str(ex)}"
                        )

                try:
                    maintenance_task.maintain_study_rw_storage_folders(
                        create_audit_folder=False
                    )
                except Exception as ex:
                    print(
                        f"Maintain task could not be completed for {study_id}. {str(ex)}"
                ...
                print(
                    f"{study_id} maintenance has been completed. Elapsed time in seconds: {int((task_done_time - start) * 100) / 100.0} "
                )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 8 and sys.argv[8]:
        skip_if_metadata_is_sync = (
            False if sys.argv[8].lower().startswith("false") else True
        )
